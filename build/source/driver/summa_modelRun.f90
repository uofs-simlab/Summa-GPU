...
contains
 ...
 USE vegPhenlgy_module,only:vegPhenlgy,vegPhenlgy_d                          ! module to compute vegetation phenology
 ...
 USE globalData,only:elapsedPhysics      
 use omp_lib                       ! elapsed time for the initialization
...
!  integer(i4b) :: nGRU2 = 1
 integer(i4b) :: im
  real(rkind),device          :: fracJulDay_d          ! fractional julian days since the start of year
 integer(i4b),device         :: yearLength_d          ! number of days in the current year
 real(rkind),device :: notUsed_canopyDepth_d(summa1_struc%nGRU), notUsed_exposedVAI_d(summa1_struc%nGRU)
fracJulDay_d = fracJulDay
yearLength_d = yearLength
!  nGRU2 = summa1_struc%nGRU
 ! ---------------------------------------------------------------------------------------
 ...
 summaVars: associate(&
  ...
  typeStruct           => summa1_struc%typeStruct_d          , & ! x%gru(:)%hru(:)%var(:)     -- local classification of soil veg etc. for each HRU
  ...
  ! run time variables
  greenVegFrac_monthly => summa1_struc%greenVegFrac_monthly_d, & ! fraction of green vegetation in each month (0-1)
 ...
 if(modelTimeStep==1)then
  ...
  do iGRU=1,summa1_struc%nGRU
    ...
    associate(scalarGreenVegFraction => diagStruct%scalarGreenVegFraction)
        !$cuf kernel do(1) <<<*,*>>>
    do jGRU=1,summa1_struc%nGRU
 ...
 allocate(timeGRU(nGRU), timeGRUstart(nGRU), timeGRUcompleted(nGRU), ixExpense(nGRU), stat=err)
 ...
 do jGRU=1,1  ! loop through GRUs
  ...
  call run_oneGRU(&
                  ...
                  summa1_struc%idStruct%gru(iGRU),           & ! intent(in):    local classification of soil veg etc. for each HRU
                  attrStruct,         & ! intent(in):    local attributes for each HRU
                  summa1_struc%lookupStruct%gru(iGRU),       & ! intent(in):    local lookup tables for each HRU
                  summa1_struc%tables, summa1_struc%decisions, summa1_struc%veg_param, summa1_struc%nGRU, &

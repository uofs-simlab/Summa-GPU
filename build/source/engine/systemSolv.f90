...
use initialize_device,only:allocate_device_flux_prev,deallocate_device_flux_data
  ...
  real(rkind),intent(out),device         :: resSink(nState,nGRU)               ! additional terms in the residual vector homegrown solver
  real(qp),intent(out),device            :: resVec(nState,nGRU)    ! NOTE: qp  ! residual vector
  real(rkind),intent(out),device         :: untappedMelt(:,:)               ! un-tapped melt energy (J m-3 s-1)
  ! output: balances (only computed at this level for ida)
  real(rkind),intent(out),device         :: balance(nState,nGRU)               ! balance per state
  ! output: model control
  ...
  real(rkind),device                     :: stateVecNew(nState,nGRU)           ! new state vector (mixed units)
  real(rkind),device                     :: fluxVec0(nState,nGRU)              ! flux vector (mixed units)
  real(rkind),device                     :: dMat(nState,nGRU)                  ! diagonal matrix (excludes flux derivatives)
  real(qp),device                        :: sMul(nState,nGRU)    ! NOTE: qp    ! multiplier for state vector for the residual calculations
  real(rkind),device                     :: rAdd(nState,nGRU)                  ! additional terms in the residual vector

...
! ************************************************************************************************************************
attributes(host,device) function T2enthalpy_snwWat(Tk,BulkDenWater,fc_param)
  ! -------------------------------------------------------------------------------------------------------------------------
  implicit none
  ! declare dummy variables
...
! ************************************************************************************************************************
attributes(host,device) subroutine T2enthTemp_veg(&
...
! ************************************************************************************************************************
attributes(host,device) subroutine T2enthTemp_snow(&
                      snowfrz_scale,          & ! intent(in):  scaling parameter for the snow freezing curve  (K-1)
...
subroutine enthTemp_or_enthalpy(&
  ...
  associate(&
   ...
   ! canopy depth
    ...
    !$cuf kernel do(2) <<<*,*>>>
    do iGRU=1,nGRU
    do iState=1,size(ixDomainType_subset,1)
      ...
      ixControlIndex = ixControlVolume(iState,iGRU)  ! index within a given domain

      ! check an energy state
      if(ixStateType(iState,iGRU)==iname_nrgCanair .or. ixStateType(iState,iGRU)==iname_nrgCanopy .or. ixStateType(iState,iGRU)==iname_nrgLayer)then
...
! ************************************************************************************************************************
attributes(device) subroutine enthalpy2T_soil(&
                      computJac,                & ! intent(in):    flag if computing for Jacobian update
                      use_lookup,               & ! intent(in):    flag to use the lookup table for soil enthalpy
                      soil_dens_intr,           & ! intent(in):    intrinsic soil density (kg m-3)
                      vGn_alpha,                & ! intent(in):    van Genutchen "alpha" parameter
...
!----------------------------------------------------------------------
 attributes(host,device) function hyp_2F1_real(a_real, b_real, c_real, z_real)
  !--------------------------------------------------------------------

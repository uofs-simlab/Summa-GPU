...
contains
 ...
 ! ************************************************************************************************
 attributes(device) subroutine vegSWavRad(&
                    ...
                    !    prog_data,                    & ! intent(inout): model prognostic variables for a local HRU
                       ...
                       scalarSnowAlbedo,&
                       scalarGroundTemp,scalarCanopyTemp, &
...
!  type(var_dlength),intent(inout)    :: prog_data                      ! model prognostic variables for a local HRU
 ...
 real(rkind),intent(inout) :: scalarGroundTemp,scalarCanopyTemp
!  type(var_dlength),intent(inout)    :: diag_data                      ! model diagnostic variables for a local HRU
 real(rkind),intent(inout) :: scalarGroundSnowFraction
 real(rkind),intent(inout) :: scalarSnowAge,scalarCosZenith,spectralSnowAlbedoDirect(:)
 real(rkind),intent(inout) :: scalarExposedLAI,scalarExposedSAI,scalarCanopyWetFraction
 real(rkind),intent(inout) :: scalarCanopySunlitFraction,scalarCanopySunlitLAI,scalarCanopyShadedLAI
...
!  type(var_dlength),intent(inout)    :: flux_data                      ! model flux variables
 real(rkind),intent(inout) :: scalarSnowfall,spectralIncomingDirect(:),spectralIncomingDiffuse(:)
 ...
 select case(ix_canopySrad)
  ...
  case(noah_mp)
   ...
   call radiation(&
                  ...
                  scalarWithinCanopyGapFraction, &
                        betais, betads, omegas, xl, hvb, hvt, rc, opt_rad, &
                        albsat, albdry, alblak, &
                        opt_alb, rhol, rhos, taul, taus          ) ! intent(out): within canopy gap fraction for beam (-)
  ...
  case(CLM_2stream,UEB_2stream,NL_scatter,BeersLaw)
   ...
   call canopy_SW(&
                  ...
                  scalarCanopyShadedPAR,                              & ! intent(out): average absorbed par for shaded leaves (w m-2)
                  err,ALBSAT,ALBDRY,RHOL,RHOS,TAUL,TAUS,&
                      omegas,betads,betais,hvt,hvb,rc,opt_rad,xl)                                         ! intent(out): error control
...
!  end associate
 ...
 ! ************************************************************************************************
 attributes(device) subroutine canopy_SW(&
                      ! input: model control
                      vegTypeIndex,                                       & ! intent(in): index of vegetation type
...
!  USE NOAHMP_VEG_PARAMETERS, only: TAUS,TAUL                                  ! Noah-MP: stem and leaf transmittance for each wave band
 ...
 integer(i4b),intent(out)          :: err                                       ! error code
 real(rkind),intent(in) :: ALBSAT(:,:), ALBDRY(:,:)
 real(rkind),intent(in) :: RHOL(:,:),RHOS(:,:),TAUL(:,:),TAUS(:,:)
    real(rkind),intent(in) :: omegas(:), betads,betais
   real(rkind),intent(in) :: hvt(:),hvb(:),rc(:),xl(:)
   integer(i4b),intent(in) :: opt_rad
  ...
  case(CLM_2stream)
   ...
   do iBand=1,nSpecBand
    ...
    call twoStream(&
                   ...
                   scalarWithinCanopyGapFraction,&
                      betais, betads, omegas, xl, hvb, hvt, rc, opt_rad      ) ! intent(inout): within canopy gap fraction for beam (-)

...
USE var_lookup,only:iLookDERIV
...
subroutine vegNrgFlux(&
  nGRU, &
                      ...
                      firstSubStep,firstFluxCall,computeVegFlux,checkLWBalance,&
  ...
  integer(i4b) :: nGRU
  logical(lgt) :: firstSubstep, firstFluxCall,checkLWBalance
  logical(lgt),device :: computeVegFlux(:)
  real(rkind),device :: scalarCanairTempTrial(:)
  real(rkind),device :: scalarCanopyTempTrial(:)
  real(rkind),device :: mLayerTempTrial(:,:)
  real(rkind),device :: scalarCanopyIceTrial(:)
  ...
  associate(&
    ...
    upperBoundTemp                  => forc_data%airtemp_d,        & ! intent(in): [dp] temperature of the upper boundary (K) --> NOTE: use air temperature
    ...
    dCanLiq_dTcanopy                => deriv_data%dCanLiq_dTcanopy,      & ! intent(in): [dp] derivative in canopy liquid w.r.t. canopy temperature (kg m-2 K-1)
    ! input: model decisions
    ix_bcUpprTdyn                   => decisions%bcUpprTdyn,           & ! intent(in): [i4b] choice of upper boundary condition for thermodynamics
    ix_veg_traits                   => decisions%veg_traits,           & ! intent(in): [i4b] choice of parameterization for vegetation roughness length and displacement height
    ix_canopyEmis                   => decisions%canopyEmis,           & ! intent(in): [i4b] choice of parameterization for canopy emissivity
    ix_windPrfile                   => decisions%windPrfile,           & ! intent(in): [i4b] choice of canopy wind profile
    ix_astability                   => decisions%astability,           & ! intent(in): [i4b] choice of stability function
    ix_soilStress                   => decisions%soilStress,           & ! intent(in): [i4b] choice of function for the soil moisture control on stomatal resistance
    ix_groundwatr                   => decisions%groundwatr,           & ! intent(in): [i4b] choice of groundwater parameterization
    ix_stomResist                   => decisions%stomResist,           & ! intent(in): [i4b] choice of function for stomatal resistance
    ix_spatial_gw                   => decisions%spatial_gw,           & ! intent(in): [i4b] choice of groundwater representation (local, basin)
    ix_bbTempFunc                   => decisions%bbTempFunc,           & ! intent(in): [i4b] leaf temperature controls on photosynthesis + stomatal resistance
    ix_bbHumdFunc                   => decisions%bbHumdFunc,           & ! intent(in): [i4b] humidity controls on stomatal resistance
    ix_bbElecFunc                   => decisions%bbElecFunc,           & ! intent(in): [i4b] dependence of photosynthesis on PAR
    ix_bbCO2point                   => decisions%bbCO2point,           & ! intent(in): [i4b] use of CO2 compensation point to calculate stomatal resistance
    ix_bbNumerics                   => decisions%bbNumerics,           & ! intent(in): [i4b] iterative numerical solution method used in the Ball-Berry parameterization
    ix_bbAssimFnc                   => decisions%bbAssimFnc,           & ! intent(in): [i4b] controls on carbon assimilation (min function, or colimitation)
    ix_bbCanIntg8                   => decisions%bbCanIntg8,           & ! intent(in): [i4b] scaling of photosynthesis from the leaf to the canopy
    ...
    soilTypeIndex                   => type_data%soilTypeIndex,                         & ! intent(in): [i4b] soil type index
    ! input: vegetation parameters
    heightCanopyTop                 => mpar_data%heightCanopyTop_,               & ! intent(in): [dp] height at the top of the vegetation canopy (m)
    heightCanopyBottom              => mpar_data%heightCanopyBottom_,            & ! intent(in): [dp] height at the bottom of the vegetation canopy (m)
    canopyWettingFactor             => mpar_data%canopyWettingFactor_,           & ! intent(in): [dp] maximum wetted fraction of the canopy (-)
    canopyWettingExp                => mpar_data%canopyWettingExp_,              & ! intent(in): [dp] exponent in canopy wetting function (-)
    scalarCanopyIceMax              => diag_data%scalarCanopyIceMax,             & ! intent(in): [dp] maximum interception storage capacity for ice (kg m-2)
    scalarCanopyLiqMax              => diag_data%scalarCanopyLiqMax,             & ! intent(in): [dp] maximum interception storage capacity for liquid water (kg m-2)
    ! input: vegetation phenology
    scalarLAI                       => diag_data%scalarLAI,                      & ! intent(in): [dp] one-sided leaf area index (m2 m-2)
    scalarSAI                       => diag_data%scalarSAI,                      & ! intent(in): [dp] one-sided stem area index (m2 m-2)
    scalarExposedLAI                => diag_data%scalarExposedLAI,               & ! intent(in): [dp] exposed leaf area index after burial by snow (m2 m-2)
    scalarExposedSAI                => diag_data%scalarExposedSAI,               & ! intent(in): [dp] exposed stem area index after burial by snow (m2 m-2)
    ...
    z0Snow                          => mpar_data%z0Snow_,                        & ! intent(in): [dp] roughness length of snow (m)
    z0Soil                          => mpar_data%z0Soil_,                        & ! intent(in): [dp] roughness length of soil (m)
    z0CanopyParam                   => mpar_data%z0Canopy_,                      & ! intent(in): [dp] roughness length of the canopy (m)
    zpdFraction                     => mpar_data%zpdFraction_,                   & ! intent(in): [dp] zero plane displacement / canopy height (-)
    critRichNumber                  => mpar_data%critRichNumber_,                & ! intent(in): [dp] critical value for the bulk Richardson number where turbulence ceases (-)
    Louis79_bparam                  => mpar_data%Louis79_bparam_,                & ! intent(in): [dp] parameter in Louis (1979) stability function
    Louis79_cStar                   => mpar_data%Louis79_cStar_,                 & ! intent(in): [dp] parameter in Louis (1979) stability function
    Mahrt87_eScale                  => mpar_data%Mahrt87_eScale_,                & ! intent(in): [dp] exponential scaling factor in the Mahrt (1987) stability function
    windReductionParam              => mpar_data%windReductionParam_,            & ! intent(in): [dp] canopy wind reduction parameter (-)
    leafExchangeCoeff               => mpar_data%leafExchangeCoeff_,             & ! intent(in): [dp] turbulent exchange coeff between canopy surface and canopy air ( m s-(1/2) )
    leafDimension                   => mpar_data%leafDimension_,                 & ! intent(in): [dp] characteristic leaf dimension (m)
    ! input: soil stress parameters
    theta_sat                       => mpar_data%theta_sat_,                     & ! intent(in): [dp] soil porosity (-)
    theta_res                       => mpar_data%theta_res_,                     & ! intent(in): [dp] residual volumetric liquid water content (-)
    plantWiltPsi                    => mpar_data%plantWiltPsi_,                  & ! intent(in): [dp] matric head at wilting point (m)
    soilStressParam                 => mpar_data%soilStressParam_,               & ! intent(in): [dp] parameter in the exponential soil stress function (-)
    critSoilWilting                 => mpar_data%critSoilWilting_,               & ! intent(in): [dp] critical vol. liq. water content when plants are wilting (-)
    critSoilTranspire               => mpar_data%critSoilTranspire_,             & ! intent(in): [dp] critical vol. liq. water content when transpiration is limited (-)
    critAquiferTranspire            => mpar_data%critAquiferTranspire_,          & ! intent(in): [dp] critical aquifer storage value when transpiration is limited (m)
    minStomatalResistance           => mpar_data%minStomatalResistance_,         & ! intent(in): [dp] mimimum stomatal resistance (s m-1)
    ! input: forcing at the upper boundary
    mHeight                         => diag_data%scalarAdjMeasHeight,            & ! intent(in): [dp] measurement height, adjusted to be above vegetation canopy and snow (m)
    airtemp                         => forc_data%airtemp_d,                              & ! intent(in): [dp] air temperature at adjusted measurement height (K)
    windspd                         => forc_data%windspd_d,                              & ! intent(in): [dp] wind speed at adjusted measurement height (m s-1)
    airpres                         => forc_data%airpres_d,                              & ! intent(in): [dp] air pressure at adjusted measurement height (Pa)
    LWRadAtm                        => forc_data%LWRadAtm_d,                             & ! intent(in): [dp] downwelling longwave radiation at the upper boundary (W m-2)
    scalarVPair                     => diag_data%scalarVPair,                    & ! intent(in): [dp] vapor pressure at adjusted measurement height (Pa)
    ...
    scalarTwetbulb                  => diag_data%scalarTwetbulb,                 & ! intent(in): [dp] wetbulb temperature (K)
    scalarRainfall                  => flux_data%ixScalarRainfall,                 & ! intent(in): [dp] computed rainfall rate (kg m-2 s-1)
    scalarSnowfall                  => flux_data%ixScalarSnowfall,                 & ! intent(in): [dp] computed snowfall rate (kg m-2 s-1)
    scalarThroughfallRain           => flux_data%ixScalarThroughfallRain,          & ! intent(in): [dp] rainfall through the vegetation canopy (kg m-2 s-1)
    scalarThroughfallSnow           => flux_data%ixScalarThroughfallSnow,          & ! intent(in): [dp] snowfall through the vegetation canopy (kg m-2 s-1)
    ! input: water storage
    ! NOTE: soil stress only computed at the start of the substep (firstFluxCall=.true.)
    scalarSWE                       => prog_data%scalarSWE,                      & ! intent(in): [dp]    snow water equivalent on the ground (kg m-2)
    scalarSnowDepth                 => prog_data%scalarSnowDepth,                & ! intent(in): [dp]    snow depth on the ground surface (m)
    scalarGroundSnowFraction        => diag_data%scalarGroundSnowFraction,       & ! intent(in): [dp] fraction of ground covered with snow (-)
    mLayerVolFracLiq                => prog_data%mLayerVolFracLiq,                  & ! intent(in): [dp(:)] volumetric fraction of liquid water in each layer (-)
    mLayerMatricHead                => prog_data%mLayerMatricHead,                  & ! intent(in): [dp(:)] matric head in each soil layer (m)
    localAquiferStorage             => prog_data%scalarAquiferStorage,           & ! intent(in): [dp]    aquifer storage for the local column (m)
    basinAquiferStorage             => bvar_data%basin__AquiferStorage,          & ! intent(in): [dp]    aquifer storage for the single basin (m)
    ! input: shortwave radiation fluxes
    scalarCanopySunlitLAI           => diag_data%scalarCanopySunlitLAI,          & ! intent(in): [dp] sunlit leaf area (-)
    scalarCanopyShadedLAI           => diag_data%scalarCanopyShadedLAI,          & ! intent(in): [dp] shaded leaf area (-)
    scalarCanopySunlitPAR           => flux_data%ixScalarCanopySunlitPAR,          & ! intent(in): [dp] average absorbed par for sunlit leaves (w m-2)
    scalarCanopyShadedPAR           => flux_data%ixScalarCanopyShadedPAR,          & ! intent(in): [dp] average absorbed par for shaded leaves (w m-2)
    scalarCanopyAbsorbedSolar       => flux_data%ixScalarCanopyAbsorbedSolar,      & ! intent(in): [dp] solar radiation absorbed by canopy (W m-2)
    scalarGroundAbsorbedSolar       => flux_data%ixScalarGroundAbsorbedSolar,      & ! intent(in): [dp] solar radiation absorbed by ground (W m-2)
    ! output: fraction of wetted canopy area
    scalarCanopyWetFraction         => diag_data%scalarCanopyWetFraction,        & ! intent(out): [dp] fraction of canopy that is wet
    ! output: longwave radiation fluxes
    scalarCanopyEmissivity          => diag_data%scalarCanopyEmissivity,         & ! intent(out): [dp] effective emissivity of the canopy (-)
    scalarLWRadCanopy               => flux_data%ixScalarLWRadCanopy,              & ! intent(out): [dp] longwave radiation emitted from the canopy (W m-2)
    scalarLWRadGround               => flux_data%ixScalarLWRadGround,              & ! intent(out): [dp] longwave radiation emitted at the ground surface (W m-2)
    scalarLWRadUbound2Canopy        => flux_data%ixScalarLWRadUbound2Canopy,       & ! intent(out): [dp] downward atmospheric longwave radiation absorbed by the canopy (W m-2)
    scalarLWRadUbound2Ground        => flux_data%ixScalarLWRadUbound2Ground,       & ! intent(out): [dp] downward atmospheric longwave radiation absorbed by the ground (W m-2)
    scalarLWRadUbound2Ubound        => flux_data%ixScalarLWRadUbound2Ubound,       & ! intent(out): [dp] atmospheric radiation reflected by the ground and lost thru upper boundary (W m-2)
    scalarLWRadCanopy2Ubound        => flux_data%ixScalarLWRadCanopy2Ubound,       & ! intent(out): [dp] longwave radiation emitted from canopy lost thru upper boundary (W m-2)
    scalarLWRadCanopy2Ground        => flux_data%ixScalarLWRadCanopy2Ground,       & ! intent(out): [dp] longwave radiation emitted from canopy absorbed by the ground (W m-2)
    scalarLWRadCanopy2Canopy        => flux_data%ixScalarLWRadCanopy2Canopy,       & ! intent(out): [dp] canopy longwave reflected from ground and absorbed by the canopy (W m-2)
    scalarLWRadGround2Ubound        => flux_data%ixScalarLWRadGround2Ubound,       & ! intent(out): [dp] longwave radiation emitted from ground lost thru upper boundary (W m-2)
    scalarLWRadGround2Canopy        => flux_data%ixScalarLWRadGround2Canopy,       & ! intent(out): [dp] longwave radiation emitted from ground and absorbed by the canopy (W m-2)
    scalarLWNetCanopy               => flux_data%ixScalarLWNetCanopy,              & ! intent(out): [dp] net longwave radiation at the canopy (W m-2)
    scalarLWNetGround               => flux_data%ixScalarLWNetGround,              & ! intent(out): [dp] net longwave radiation at the ground surface (W m-2)
    scalarLWNetUbound               => flux_data%ixScalarLWNetUbound,              & ! intent(out): [dp] net longwave radiation at the upper boundary (W m-2)
    ! output: aerodynamic resistance
    scalarZ0Canopy                  => diag_data%scalarZ0Canopy,                 & ! intent(out): [dp] roughness length of the canopy (m)
    scalarWindReductionFactor       => diag_data%scalarWindReductionFactor,      & ! intent(out): [dp] canopy wind reduction factor (-)
    scalarZeroPlaneDisplacement     => diag_data%scalarZeroPlaneDisplacement,    & ! intent(out): [dp] zero plane displacement (m)
    scalarRiBulkCanopy              => diag_data%scalarRiBulkCanopy,             & ! intent(out): [dp] bulk Richardson number for the canopy (-)
    scalarRiBulkGround              => diag_data%scalarRiBulkGround,             & ! intent(out): [dp] bulk Richardson number for the ground surface (-)
    scalarEddyDiffusCanopyTop       => flux_data%ixScalarEddyDiffusCanopyTop,      & ! intent(out): [dp] eddy diffusivity for heat at the top of the canopy (m2 s-1)
    scalarFrictionVelocity          => flux_data%ixScalarFrictionVelocity,         & ! intent(out): [dp] friction velocity (m s-1)
    scalarWindspdCanopyTop          => flux_data%ixScalarWindspdCanopyTop,         & ! intent(out): [dp] windspeed at the top of the canopy (m s-1)
    scalarWindspdCanopyBottom       => flux_data%ixScalarWindspdCanopyBottom,      & ! intent(out): [dp] windspeed at the height of the bottom of the canopy (m s-1)
    scalarLeafResistance            => flux_data%ixScalarLeafResistance,           & ! intent(out): [dp] mean leaf boundary layer resistance per unit leaf area (s m-1)
    scalarGroundResistance          => flux_data%ixScalarGroundResistance,         & ! intent(out): [dp] below canopy aerodynamic resistance (s m-1)
    scalarCanopyResistance          => flux_data%ixScalarCanopyResistance,         & ! intent(out): [dp] above canopy aerodynamic resistance (s m-1)
    ! input/output: soil resistance -- intent(in) and intent(inout) because only called at the first flux call
    mLayerRootDensity               => diag_data%mLayerRootDensity,                 & ! intent(in):    [dp] root density in each layer (-)
    scalarAquiferRootFrac           => diag_data%scalarAquiferRootFrac,          & ! intent(in):    [dp] fraction of roots below the lowest soil layer (-)
    scalarTranspireLim              => diag_data%scalarTranspireLim,             & ! intent(inout): [dp] weighted average of the transpiration limiting factor (-)
    mLayerTranspireLim              => diag_data%mLayerTranspireLim,                & ! intent(inout): [dp] transpiration limiting factor in each layer (-)
    scalarTranspireLimAqfr          => diag_data%scalarTranspireLimAqfr,         & ! intent(inout): [dp] transpiration limiting factor for the aquifer (-)
    scalarSoilRelHumidity           => diag_data%scalarSoilRelHumidity,          & ! intent(inout): [dp] relative humidity in the soil pores [0-1]
    scalarSoilResistance            => flux_data%ixScalarSoilResistance,           & ! intent(inout): [dp] resistance from the soil (s m-1)
    ! input/output: stomatal resistance -- intent(inout) because only called at the first flux call
    scalarStomResistSunlit          => flux_data%ixScalarStomResistSunlit,         & ! intent(inout): [dp] stomatal resistance for sunlit leaves (s m-1)
    scalarStomResistShaded          => flux_data%ixScalarStomResistShaded,         & ! intent(inout): [dp] stomatal resistance for shaded leaves (s m-1)
    scalarPhotosynthesisSunlit      => flux_data%ixScalarPhotosynthesisSunlit,     & ! intent(inout): [dp] sunlit photosynthesis (umolco2 m-2 s-1)
    scalarPhotosynthesisShaded      => flux_data%ixScalarPhotosynthesisShaded,     & ! intent(inout): [dp] shaded photosynthesis (umolco2 m-2 s-1)
    ! output: turbulent heat fluxes
    scalarLatHeatSubVapCanopy       => diag_data%scalarLatHeatSubVapCanopy,      & ! intent(inout): [dp] latent heat of sublimation/vaporization for the vegetation canopy (J kg-1)
    scalarLatHeatSubVapGround       => diag_data%scalarLatHeatSubVapGround,      & ! intent(inout): [dp] latent heat of sublimation/vaporization for the ground surface (J kg-1)
    scalarSatVP_canopyTemp          => diag_data%scalarSatVP_CanopyTemp,         & ! intent(out):   [dp] saturation vapor pressure at the temperature of the vegetation canopy (Pa)
    scalarSatVP_groundTemp          => diag_data%scalarSatVP_GroundTemp,         & ! intent(out):   [dp] saturation vapor pressure at the temperature of the ground surface (Pa)
    scalarSenHeatTotal              => flux_data%ixScalarSenHeatTotal,             & ! intent(out):   [dp] sensible heat from the canopy air space to the atmosphere (W m-2)
    scalarSenHeatCanopy             => flux_data%ixScalarSenHeatCanopy,            & ! intent(out):   [dp] sensible heat flux from the canopy to the canopy air space (W m-2)
    scalarSenHeatGround             => flux_data%ixScalarSenHeatGround,            & ! intent(out):   [dp] sensible heat flux from ground surface below vegetation (W m-2)
    scalarLatHeatTotal              => flux_data%ixScalarLatHeatTotal,             & ! intent(out):   [dp] latent heat from the canopy air space to the atmosphere (W m-2)
    scalarLatHeatCanopyEvap         => flux_data%ixScalarLatHeatCanopyEvap,        & ! intent(out):   [dp] latent heat flux for evaporation from the canopy to the canopy air space (W m-2)
    scalarLatHeatCanopyTrans        => flux_data%ixScalarLatHeatCanopyTrans,       & ! intent(out):   [dp] latent heat flux for transpiration from the canopy to the canopy air space (W m-2)
    scalarLatHeatGround             => flux_data%ixScalarLatHeatGround,            & ! intent(out):   [dp] latent heat flux from ground surface below vegetation (W m-2)
    ! output: advective heat fluxes
    scalarCanopyAdvectiveHeatFlux   => flux_data%ixScalarCanopyAdvectiveHeatFlux,  & ! intent(out): [dp] heat advected to the canopy surface with rain + snow (W m-2)
    scalarGroundAdvectiveHeatFlux   => flux_data%ixScalarGroundAdvectiveHeatFlux,  & ! intent(out): [dp] heat advected to the ground surface with throughfall (W m-2)
    ! output: mass fluxes
    scalarCanopySublimation         => flux_data%ixScalarCanopySublimation,        & ! intent(out): [dp] canopy sublimation/frost (kg m-2 s-1)
    scalarSnowSublimation           => flux_data%ixScalarSnowSublimation,          & ! intent(out): [dp] snow sublimation/frost -- below canopy or non-vegetated (kg m-2 s-1)
    ! input/output: canopy air space variables
    scalarVP_CanopyAir              => diag_data%scalarVP_CanopyAir,             & ! intent(inout): [dp] vapor pressure of the canopy air space (Pa)
    scalarCanopyStabilityCorrection => diag_data%scalarCanopyStabilityCorrection,& ! intent(inout): [dp] stability correction for the canopy (-)
    scalarGroundStabilityCorrection => diag_data%scalarGroundStabilityCorrection,& ! intent(inout): [dp] stability correction for the ground surface (-)
    ! output: liquid water fluxes
    scalarCanopyTranspiration       => flux_data%ixScalarCanopyTranspiration,      & ! intent(out): [dp] canopy transpiration (kg m-2 s-1)
    scalarCanopyEvaporation         => flux_data%ixScalarCanopyEvaporation,        & ! intent(out): [dp] canopy evaporation/condensation (kg m-2 s-1)
    scalarGroundEvaporation         => flux_data%ixScalarGroundEvaporation,        & ! intent(out): [dp] ground evaporation/condensation -- below canopy or non-vegetated (kg m-2 s-1)
    ! output: derived fluxes
    scalarTotalET                   => flux_data%ixScalarTotalET,                  & ! intent(out): [dp] total ET (kg m-2 s-1)
    scalarNetRadiation              => flux_data%ixScalarNetRadiation,             & ! intent(out): [dp] net radiation (W m-2)
    ! output: liquid water fluxes associated with evaporation/transpiration (needed for coupling)
    returnCanopyTranspiration => flux_data%ixScalarCanopyTranspiration,  & ! intent(out): [dp] canopy transpiration (kg m-2 s-1)
    returnCanopyEvaporation   => flux_data%ixScalarCanopyEvaporation,    & ! intent(out): [dp] canopy evaporation/condensation (kg m-2 s-1)
    returnGroundEvaporation   => flux_data%ixScalarGroundEvaporation,    & ! intent(out): [dp] ground evaporation/condensation -- below canopy or non-vegetated (kg m-2 s-1)
    ! output: fluxes
    canairNetFlux => flux_data%ixScalarCanairNetNrgFlux,                 & ! intent(out): [dp] net energy flux for the canopy air space (W m-2)
    canopyNetFlux => flux_data%ixScalarCanopyNetNrgFlux,                 & ! intent(out): [dp] net energy flux for the vegetation canopy (W m-2)
    groundNetFlux => flux_data%ixScalarGroundNetNrgFlux,                 & ! intent(out): [dp] net energy flux for the ground surface (W m-2)
    ! output: energy flux derivatives
    dCanairNetFlux_dCanairTemp => deriv_data%dCanairNetFlux_dCanairTemp,& ! intent(out): [dp] derivative in net canopy air space flux w.r.t. canopy air temperature (W m-2 K-1)
    dCanairNetFlux_dCanopyTemp => deriv_data%dCanairNetFlux_dCanopyTemp,& ! intent(out): [dp] derivative in net canopy air space flux w.r.t. canopy temperature (W m-2 K-1)
    dCanairNetFlux_dGroundTemp => deriv_data%dCanairNetFlux_dGroundTemp,& ! intent(out): [dp] derivative in net canopy air space flux w.r.t. ground temperature (W m-2 K-1)
    dCanopyNetFlux_dCanairTemp => deriv_data%dCanopyNetFlux_dCanairTemp,& ! intent(out): [dp] derivative in net canopy flux w.r.t. canopy air temperature (W m-2 K-1)
    dCanopyNetFlux_dCanopyTemp => deriv_data%dCanopyNetFlux_dCanopyTemp,& ! intent(out): [dp] derivative in net canopy flux w.r.t. canopy temperature (W m-2 K-1)
    dCanopyNetFlux_dGroundTemp => deriv_data%dCanopyNetFlux_dGroundTemp,& ! intent(out): [dp] derivative in net canopy flux w.r.t. ground temperature (W m-2 K-1)
    dGroundNetFlux_dCanairTemp => deriv_data%dGroundNetFlux_dCanairTemp,& ! intent(out): [dp] derivative in net ground flux w.r.t. canopy air temperature (W m-2 K-1)
    dGroundNetFlux_dCanopyTemp => deriv_data%dGroundNetFlux_dCanopyTemp,& ! intent(out): [dp] derivative in net ground flux w.r.t. canopy temperature (W m-2 K-1)
    dGroundNetFlux_dGroundTemp => deriv_data%dGroundNetFlux_dGroundTemp,& ! intent(out): [dp] derivative in net ground flux w.r.t. ground temperature (W m-2 K-1)
    ! output: liquid flux derivatives (canopy evap)
    dCanopyEvaporation_dCanWat  => deriv_data%dCanopyEvaporation_dCanWat,  & ! intent(out): [dp] derivative in canopy evaporation w.r.t. canopy total water content (s-1)
    dCanopyEvaporation_dTCanair => deriv_data%dCanopyEvaporation_dTCanair, & ! intent(out): [dp] derivative in canopy evaporation w.r.t. canopy air temperature (kg m-2 s-1 K-1)
    dCanopyEvaporation_dTCanopy => deriv_data%dCanopyEvaporation_dTCanopy, & ! intent(out): [dp] derivative in canopy evaporation w.r.t. canopy temperature (kg m-2 s-1 K-1)
    dCanopyEvaporation_dTGround => deriv_data%dCanopyEvaporation_dTGround, & ! intent(out): [dp] derivative in canopy evaporation w.r.t. ground temperature (kg m-2 s-1 K-1)
    ! output: liquid flux derivatives (ground evap)
    dGroundEvaporation_dCanWat  => deriv_data%dGroundEvaporation_dCanWat,  & ! intent(out): [dp] derivative in ground evaporation w.r.t. canopy total water content (s-1)
    dGroundEvaporation_dTCanair => deriv_data%dGroundEvaporation_dTCanair, & ! intent(out): [dp] derivative in ground evaporation w.r.t. canopy air temperature (kg m-2 s-1 K-1)
    dGroundEvaporation_dTCanopy => deriv_data%dGroundEvaporation_dTCanopy, & ! intent(out): [dp] derivative in ground evaporation w.r.t. canopy temperature (kg m-2 s-1 K-1)
    dGroundEvaporation_dTGround => deriv_data%dGroundEvaporation_dTGround, & ! intent(out): [dp] derivative in ground evaporation w.r.t. ground temperature (kg m-2 s-1 K-1)
    ! output: transpiration derivatives
    dCanopyTrans_dCanWat  => deriv_data%dCanopyTrans_dCanWat,          & ! intent(out): [dp] derivative in canopy transpiration w.r.t. canopy total water content (s-1)
    dCanopyTrans_dTCanair => deriv_data%dCanopyTrans_dTCanair,         & ! intent(out): [dp] derivative in canopy transpiration w.r.t. canopy air temperature (kg m-2 s-1 K-1)
    dCanopyTrans_dTCanopy => deriv_data%dCanopyTrans_dTCanopy,         & ! intent(out): [dp] derivative in canopy transpiration w.r.t. canopy temperature (kg m-2 s-1 K-1)
    dCanopyTrans_dTGround => deriv_data%dCanopyTrans_dTGround,         & ! intent(out): [dp] derivative in canopy transpiration w.r.t. ground temperature (kg m-2 s-1 K-1)
    ! output: cross derivative terms
    dCanopyNetFlux_dCanWat => deriv_data%dCanopyNetFlux_dCanWat,       & ! intent(out): [dp] derivative in net canopy fluxes w.r.t. canopy total water content (J kg-1 s-1)
    dGroundNetFlux_dCanWat => deriv_data%dGroundNetFlux_dCanWat,        & ! intent(out): [dp] derivative in net ground fluxes w.r.t. canopy total water content (J kg-1 s-1)
    iLayerThermalC => diag_data%iLayerThermalC, &
...
!  minStomatalResistance           => mpar_data%var(iLookPARAM%minStomatalResistance)%dat(1),         & ! intent(in): [dp] mimimum stomatal resistance (s m-1)
 vcmax25_canopyTop               => mpar_data%vcmax25_canopyTop_,             & ! intent(in): [dp] potential carboxylation rate at 25 degrees C at the canopy top (umol co2 m-2 s-1)
  Kc25                            => mpar_data%Kc25_,                          & ! intent(in): [dp] Michaelis-Menten constant for CO2 at 25 degrees C (umol mol-1)
 Ko25                            => mpar_data%Ko25_,                          & ! intent(in): [dp] Michaelis-Menten constant for O2 at 25 degrees C (mol mol-1)
 Kc_qFac                         => mpar_data%Kc_qFac_,                       & ! intent(in): [dp] factor in the q10 function defining temperature controls on Kc (-)
 Ko_qFac                         => mpar_data%Ko_qFac_,                       & ! intent(in): [dp] factor in the q10 function defining temperature controls on Ko (-)
 kc_Ha                           => mpar_data%kc_Ha_,                         & ! intent(in): [dp] activation energy for the Michaelis-Menten constant for CO2 (J mol-1)
 ko_Ha                           => mpar_data%ko_Ha_,                         & ! intent(in): [dp] activation energy for the Michaelis-Menten constant for O2 (J mol-1)
 vcmax_qFac                      => mpar_data%vcmax_qFac_,                    & ! intent(in): [dp] factor in the q10 function defining temperature controls on vcmax (-)
 vcmax_Ha                        => mpar_data%vcmax_Ha_,                      & ! intent(in): [dp] activation energy in the vcmax function (J mol-1)
 vcmax_Hd                        => mpar_data%vcmax_Hd_,                      & ! intent(in): [dp] deactivation energy in the vcmax function (J mol-1)
 vcmax_Sv                        => mpar_data%vcmax_Sv_,                      & ! intent(in): [dp] entropy term in the vcmax function (J mol-1 K-1)
 vcmax_Kn                        => mpar_data%vcmax_Kn_,                      & ! intent(in): [dp] foliage nitrogen decay coefficient (-)
 jmax25_scale                    => mpar_data%jmax25_scale_,                  & ! intent(in): [dp] scaling factor to relate jmax25 to vcmax25 (-)
 jmax_Ha                         => mpar_data%jmax_Ha_,                       & ! intent(in): [dp] activation energy in the jmax function (J mol-1)
 jmax_Hd                         => mpar_data%jmax_Hd_,                       & ! intent(in): [dp] deactivation energy in the jmax function (J mol-1)
 jmax_Sv                         => mpar_data%jmax_Sv_,                       & ! intent(in): [dp] entropy term in the jmax function (J mol-1 K-1)
 fractionJ                       => mpar_data%fractionJ_,                     & ! intent(in): [dp] fraction of light lost by other than the chloroplast lamellae (-)
 quantamYield                    => mpar_data%quantamYield_,                  & ! intent(in): [dp] quantam yield (mol e mol-1 q)
 vpScaleFactor                   => mpar_data%vpScaleFactor_,                 & ! intent(in): [dp] vapor pressure scaling factor in stomatal conductance function (Pa)
 cond2photo_slope                => mpar_data%cond2photo_slope_,              & ! intent(in): [dp] slope of conductance-photosynthesis relationship (-)
 minStomatalConductance          => mpar_data%minStomatalConductance_,        & ! intent(in): [dp] mimimum stomatal conductance (umol H2O m-2 s-1)
 bp_noahmp => veg_param % bp, &
 mp_noahmp => veg_param % mp, &
 c3psn_noahmp => veg_param % c3psn, &
 avcmx_noahmp => veg_param % avcmx, &
 vcmx25_noahmp => veg_param % vcmx25, &
 ako_noahmp => veg_param % ako, &
 ko25_noahmp => veg_param % ko25, &
 akc_noahmp => veg_param % akc, &
 kc25_noahmp => veg_param % kc25, &
 qe25_noahmp => veg_param % qe25, &
 folnmx_noahmp => veg_param % folnmx, &
 rsmin_noahmp => veg_param % rsmin_d, &
 hs_noahmp => veg_param % hs_d, &
 topt_noahmp => veg_param % topt_d, &
 rsmax_noahmp => veg_param % rsmax_d, &
 rgl_noahmp => veg_param % rgl_d, &
    ...
    call vegNrgFlux_kernel<<<blocks,threads>>>(nGRU,ixCasNrg,ixVegNrg,ixTopNrg,firstSubStep,firstFluxCall,computeVegFlux,checkLWBalance,&
  upperBoundTemp,scalarCanairTempTrial,scalarCanopyTempTrial,mLayerTempTrial,scalarCanopyIceTrial,scalarCanopyLiqTrial,&
  dCanLiq_dTcanopy,&
  ix_bcUpprTdyn,ix_veg_traits,ix_canopyEmis,ix_windPrfile,ix_astability,ix_soilStress,ix_groundwatr,ix_stomResist,ix_spatial_gw,&
  ix_bbTempFunc,ix_bbHumdFunc,ix_bbElecFunc,ix_bbCO2point,ix_bbNumerics,ix_bbAssimFnc,ix_bbCanIntg8, &
  nSnow,nSoil,nLayers,&
  vegTypeIndex,soilTypeIndex,&
  heightCanopyTop,heightCanopyBottom,canopyWettingFactor,canopyWettingExp,scalarCanopyIceMax,scalarCanopyLiqMax,&
scalarLAI,scalarSAI,scalarExposedLAI,scalarExposedSAI,scalarGrowingSeasonIndex,scalarFoliageNitrogenFactor,&
z0Snow,z0Soil,z0CanopyParam,zpdFraction,critRichNumber,Louis79_bparam,Louis79_cStar,Mahrt87_eScale,windReductionParam,leafExchangeCoeff,leafDimension,&
theta_sat,theta_res,plantWiltPsi,soilStressParam,critSoilWilting,critSoilTranspire,critAquiferTranspire,minStomatalResistance,&
mHeight,airtemp,windspd,airpres,LWRadAtm,scalarVPair,scalarO2air,scalarCO2air,scalarTwetbulb,scalarRainfall,scalarSnowfall,scalarThroughfallRain,scalarThroughfallSnow,&
scalarSWE,scalarSnowDepth,scalarGroundSnowFraction,mLayerVolFracLiq,mLayerMatricHead,localAquiferStorage,basinAquiferStorage,&
scalarCanopySunlitLAI,scalarCanopyShadedLAI,scalarCanopySunlitPAR,scalarCanopyShadedPAR,scalarCanopyAbsorbedSolar,scalarGroundAbsorbedSolar,&
scalarCanopyWetFraction, &
scalarCanopyEmissivity,scalarLWRadCanopy,scalarLWRadGround,scalarLWRadUbound2Canopy,scalarLWRadUbound2Ground,scalarLWRadUbound2Ubound,scalarLWRadCanopy2Ubound,scalarLWRadCanopy2Ground,scalarLWRadCanopy2Canopy,scalarLWRadGround2Ubound,scalarLWRadGround2Canopy,scalarLWNetCanopy,scalarLWNetGround,scalarLWNetUbound,&
scalarZ0Canopy,scalarWindReductionFactor,scalarZeroPlaneDisplacement,scalarRiBulkCanopy,scalarRiBulkGround,scalarEddyDiffusCanopyTop,scalarFrictionVelocity,scalarWindspdCanopyTop,scalarWindspdCanopyBottom,scalarLeafResistance,scalarGroundResistance,scalarCanopyResistance,&
mLayerRootDensity,scalarAquiferRootFrac,scalarTranspireLim,mLayerTranspireLim,scalarTranspireLimAqfr,scalarSoilRelHumidity,scalarSoilResistance,&
scalarStomResistSunlit,scalarStomResistShaded,scalarPhotosynthesisSunlit,scalarPhotosynthesisShaded,&
scalarLatHeatSubVapCanopy,scalarLatHeatSubVapGround,scalarSatVP_canopyTemp,scalarSatVP_groundTemp,scalarSenHeatTotal,scalarSenHeatCanopy,scalarSenHeatGround,scalarLatHeatTotal,scalarLatHeatCanopyEvap,scalarLatHeatCanopyTrans,scalarLatHeatGround,&
scalarCanopyAdvectiveHeatFlux,scalarGroundAdvectiveHeatFlux,&
scalarCanopySublimation,scalarSnowSublimation,&
scalarVP_CanopyAir,scalarCanopyStabilityCorrection,scalarGroundStabilityCorrection,&
scalarCanopyTranspiration,scalarCanopyEvaporation,scalarGroundEvaporation,&
scalarTotalET,scalarNetRadiation,&
returnCanopyTranspiration,returnCanopyEvaporation,returnGroundEvaporation,&
flux_data%data, &
canairNetFlux,canopyNetFlux,groundNetFlux,&
dCanairNetFlux_dCanairTemp,dCanairNetFlux_dCanopyTemp,dCanairNetFlux_dGroundTemp,dCanopyNetFlux_dCanairTemp,dCanopyNetFlux_dCanopyTemp,dCanopyNetFlux_dGroundTemp,dGroundNetFlux_dCanairTemp,dGroundNetFlux_dCanopyTemp,dGroundNetFlux_dGroundTemp,&
dCanopyEvaporation_dCanWat,dCanopyEvaporation_dTCanair,dCanopyEvaporation_dTCanopy,dCanopyEvaporation_dTGround,&
dGroundEvaporation_dCanWat,dGroundEvaporation_dTCanair,dGroundEvaporation_dTCanopy,dGroundEvaporation_dTGround,&
dCanopyTrans_dCanWat,dCanopyTrans_dTCanair,dCanopyTrans_dTCanopy,dCanopyTrans_dTGround,&
dCanopyNetFlux_dCanWat,dGroundNetFlux_dCanWat,&
scalarIntercellularCO2Sunlit,scalarIntercellularCO2Shaded,iLayerThermalC,mLayerDepth,&
vcmax25_canopyTop,Kc25,Ko25,Kc_qFac,Ko_qFac,kc_Ha,ko_Ha,vcmax_qFac,vcmax_Ha,vcmax_Hd,vcmax_Sv,vcmax_Kn,jmax25_scale,jmax_Ha,jmax_Hd,jmax_Sv,fractionJ,quantamYield,vpScaleFactor,cond2photo_slope,minStomatalConductance,&
bp_noahmp,mp_noahmp,c3psn_noahmp,avcmx_noahmp,vcmx25_noahmp,ako_noahmp,ko25_noahmp,akc_noahmp,kc25_noahmp,qe25_noahmp,folnmx_noahmp,&
                              rsmin_noahmp,hs_noahmp,topt_noahmp,rsmax_noahmp,rgl_noahmp,minExpLogHgtFac)
...
attributes(global) subroutine vegNrgFlux_kernel(nGRU,ixCasNrg,ixVegNrg,ixTopNrg,&
firstSubStep,firstFluxCall,computeVegFlux,checkLWBalance,&
  upperBoundTemp,canairTempTrial,canopyTempTrial,groundTempTrial,canopyIceTrial,canopyLiqTrial,&
  dCanLiq_dTcanopy,&
  ix_bcUpprTdyn,ix_veg_traits,ix_canopyEmis,ix_windPrfile,ix_astability,ix_soilStress,ix_groundwatr,ix_stomResist,ix_spatial_gw,&
  ix_bbTempFunc,ix_bbHumdFunc,ix_bbElecFunc,ix_bbCO2point,ix_bbNumerics,ix_bbAssimFnc,ix_bbCanIntg8, &
  nSnow,nSoil,nLayers,&
...
bp_noahmp,mp_noahmp,c3psn_noahmp,avcmx_noahmp,vcmx25_noahmp,ako_noahmp,ko25_noahmp,akc_noahmp,kc25_noahmp,qe25_noahmp,folnmx_noahmp,&
  ...
  integer(i4b) :: ixCasNrg(:), ixVegNrg(:), ixTopNrg(:)
    ! input: model control
    logical(lgt),intent(in),value :: firstSubStep                     ! intent(in): [dp] flag to indicate if we are processing the first sub-step
    logical(lgt),intent(in),value :: firstFluxCall                    ! intent(in): [dp] flag to indicate if we are processing the first flux call
    logical(lgt),intent(in) :: computeVegFlux(:)                   ! intent(in): [dp] flag to indicate if computing fluxes over vegetation
    logical(lgt),intent(in),value :: checkLWBalance                   ! intent(in): [dp] flag to check longwave balance
    ! input: model state variables
    real(rkind),intent(inout) :: upperBoundTemp(:)                   ! intent(in): [dp] temperature of the upper boundary (K) --> NOTE: use air temperature
    real(rkind),intent(inout) :: canairTempTrial(:)                  ! intent(in): [dp] trial value of canopy air space temperature (K)
    real(rkind),intent(inout) :: canopyTempTrial(:)                  ! intent(in): [dp] trial value of canopy temperature (K)
    real(rkind),intent(inout) :: groundTempTrial(:,:)                  ! intent(in): [dp] trial value of ground temperature (K)
    real(rkind),intent(inout) :: canopyIceTrial(:)                   ! intent(in): [dp] trial value of mass of ice on the vegetation canopy (kg m-2)
    real(rkind),intent(inout) :: canopyLiqTrial(:)                   ! intent(in): [dp] trial value of mass of liquid water on the vegetation canopy (kg m-2)
    ! input: model derivatives
    real(rkind),intent(in) :: dCanLiq_dTcanopy(:) ! intent(in): [dp] derivative in canopy liquid w.r.t. canopy temperature (kg m-2 K-1)
    ! input: model decisions
    integer(i4b),intent(in) :: ix_bcUpprTdyn ! intent(in): [i4b] choice of upper boundary condition for thermodynamics
    integer(i4b),intent(in) :: ix_veg_traits ! intent(in): [i4b] choice of parameterization for vegetation roughness length and displacement height
    integer(i4b),intent(in) :: ix_canopyEmis ! intent(in): [i4b] choice of parameterization for canopy emissivity
    integer(i4b),intent(in) :: ix_windPrfile ! intent(in): [i4b] choice of canopy wind profile
    integer(i4b),intent(in) :: ix_astability ! intent(in): [i4b] choice of stability function
    integer(i4b),intent(in) :: ix_soilStress ! intent(in): [i4b] choice of function for the soil moisture control on stomatal resistance
    integer(i4b),intent(in) :: ix_groundwatr ! intent(in): [i4b] choice of groundwater parameterization
    integer(i4b),intent(in) :: ix_stomResist ! intent(in): [i4b] choice of function for stomatal resistance
    integer(i4b),intent(in) :: ix_spatial_gw ! intent(in): [i4b] choice of groundwater representation (local, basin)
    integer(i4b),intent(in) :: ix_bbTempFunc,ix_bbHumdFunc,ix_bbElecFunc,ix_bbCO2point,ix_bbNumerics,ix_bbAssimFnc,ix_bbCanIntg8
    ! input: layer geometry
    integer(i4b),intent(in) :: nSnow(:) ! intent(in): [i4b] number of snow layers
    integer(i4b),intent(in),value :: nSoil ! intent(in): [i4b] number of soil layers
    integer(i4b),intent(in) :: nLayers(:) ! intent(in): [i4b] total number of layers
    ! input: physical attributes
    integer(i4b),intent(in) :: vegTypeIndex(:)                     ! intent(in): [i4b] vegetation type index
    integer(i4b),intent(in) :: soilTypeIndex(:)                    ! intent(in): [i4b] soil type index
    ! input: vegetation parameters
    real(rkind),intent(in) :: heightCanopyTop(:)                  ! intent(in): [dp] height at the top of the vegetation canopy (m)
    real(rkind),intent(in) :: heightCanopyBottom(:)               ! intent(in): [dp] height at the bottom of the vegetation canopy (m)
    real(rkind),intent(in) :: canopyWettingFactor(:)              ! intent(in): [dp] maximum wetted fraction of the canopy (-)
    real(rkind),intent(in) :: canopyWettingExp(:)                 ! intent(in): [dp] exponent in canopy wetting function (-)
    real(rkind),intent(in) :: scalarCanopyIceMax(:)               ! intent(in): [dp] maximum interception storage capacity for ice (kg m-2)
    real(rkind),intent(in) :: scalarCanopyLiqMax(:)               ! intent(in): [dp] maximum interception storage capacity for liquid water (kg m-2)
    ! input: vegetation phenology
    real(rkind),intent(in) :: scalarLAI(:)                        ! intent(in): [dp] one-sided leaf area index (m2 m-2)
    real(rkind),intent(in) :: scalarSAI(:)                        ! intent(in): [dp] one-sided stem area index (m2 m-2)
    real(rkind),intent(in) :: scalarExposedLAI(:)                 ! intent(in): [dp] exposed leaf area index after burial by snow (m2 m-2)
    real(rkind),intent(in) :: scalarExposedSAI(:)                 ! intent(in): [dp] exposed stem area index after burial by snow (m2 m-2)
    real(rkind),intent(in) :: scalarGrowingSeasonIndex(:)         ! intent(in): [dp] growing season index (0=off, 1=on)
    real(rkind),intent(in) :: scalarFoliageNitrogenFactor(:)      ! intent(in): [dp] foliage nitrogen concentration (1.0 = saturated)
    ! input: aerodynamic resistance parameters
    real(rkind),intent(in) :: z0Snow(:)                           ! intent(in): [dp] roughness length of snow (m)
    real(rkind),intent(in) :: z0Soil(:)                           ! intent(in): [dp] roughness length of soil (m)
    real(rkind),intent(in) :: z0CanopyParam(:)                    ! intent(in): [dp] roughness length of the canopy (m)
    real(rkind),intent(in) :: zpdFraction(:)                      ! intent(in): [dp] zero plane displacement / canopy height (-)
    real(rkind),intent(in) :: critRichNumber(:)                   ! intent(in): [dp] critical value for the bulk Richardson number where turbulence ceases (-)
    real(rkind),intent(in) :: Louis79_bparam(:)                   ! intent(in): [dp] parameter in Louis (1979) stability function
    real(rkind),intent(in) :: Louis79_cStar(:)                    ! intent(in): [dp] parameter in Louis (1979) stability function
    real(rkind),intent(in) :: Mahrt87_eScale(:)                   ! intent(in): [dp] exponential scaling factor in the Mahrt (1987) stability function
    real(rkind),intent(in) :: windReductionParam(:)               ! intent(in): [dp] canopy wind reduction parameter (-)
    real(rkind),intent(in) :: leafExchangeCoeff(:)                ! intent(in): [dp] turbulent exchange coeff between canopy surface and canopy air ( m s-(1/2) )
    real(rkind),intent(in) :: leafDimension(:)                    ! intent(in): [dp] characteristic leaf dimension (m)
    ! input: soil stress parameters
    real(rkind),intent(in) :: theta_sat(:,:)                        ! intent(in): [dp] soil porosity (-)
    real(rkind),intent(in) :: theta_res(:,:)                        ! intent(in): [dp] residual volumetric liquid water content (-)
    real(rkind),intent(in) :: plantWiltPsi(:)                     ! intent(in): [dp] matric head at wilting point (m)
    real(rkind),intent(in) :: soilStressParam(:)                  ! intent(in): [dp] parameter in the exponential soil stress function (-)
    real(rkind),intent(in) :: critSoilWilting(:)                  ! intent(in): [dp] critical vol. liq. water content when plants are wilting (-)
    real(rkind),intent(in) :: critSoilTranspire(:)                ! intent(in): [dp] critical vol. liq. water content when transpiration is limited (-)
    real(rkind),intent(in) :: critAquiferTranspire(:)             ! intent(in): [dp] critical aquifer storage value when transpiration is limited (m)
    real(rkind),intent(in) :: minStomatalResistance(:)            ! intent(in): [dp] mimimum stomatal resistance (s m-1)
    ! input: forcing at the upper boundary
    real(rkind),intent(in) :: mHeight(:)                          ! intent(in): [dp] measurement height, adjusted to be above vegetation canopy and snow (m)
    real(rkind),intent(in) :: airtemp(:)                          ! intent(in): [dp] air temperature at adjusted measurement height (K)
    real(rkind),intent(in) :: windspd(:)                          ! intent(in): [dp] wind speed at adjusted measurement height (m s-1)
    real(rkind),intent(in) :: airpres(:)                          ! intent(in): [dp] air pressure at adjusted measurement height (Pa)
    real(rkind),intent(in) :: LWRadAtm(:)                         ! intent(in): [dp] downwelling longwave radiation at the upper boundary (W m-2)
    real(rkind),intent(in) :: scalarVPair(:)                      ! intent(in): [dp] vapor pressure at adjusted measurement height (Pa)
    real(rkind),intent(in) :: scalarO2air(:)                      ! intent(in): [dp] atmospheric o2 concentration (Pa)
    real(rkind),intent(in) :: scalarCO2air(:)                     ! intent(in): [dp] atmospheric co2 concentration (Pa)
    real(rkind),intent(in) :: scalarTwetbulb(:)                   ! intent(in): [dp] wetbulb temperature (K)
    integer(i4b),intent(in),value :: scalarRainfall                   ! intent(in): [dp] computed rainfall rate (kg m-2 s-1)
    integer(i4b),intent(in),value :: scalarSnowfall                   ! intent(in): [dp] computed snowfall rate (kg m-2 s-1)
    integer(i4b),intent(in),value :: scalarThroughfallRain            ! intent(in): [dp] rainfall through the vegetation canopy (kg m-2 s-1)
    integer(i4b),intent(in),value :: scalarThroughfallSnow            ! intent(in): [dp] snowfall through the vegetation canopy (kg m-2 s-1)
    ! input: water storage
    ! NOTE: soil stress only computed at the start of the substep (firstFluxCall=.true.)
    real(rkind),intent(in) :: scalarSWE(:)                        ! intent(in): [dp]    snow water equivalent on the ground (kg m-2)
    real(rkind),intent(in) :: scalarSnowDepth(:)                  ! intent(in): [dp]    snow depth on the ground surface (m)
    real(rkind),intent(in) :: scalarGroundSnowFraction(:)         ! intent(in): [dp] fraction of ground covered with snow (-)
    real(rkind),intent(in) :: mLayerVolFracLiq(:,:)                 ! intent(in): [dp(:)] volumetric fraction of liquid water in each layer (-)
    real(rkind),intent(in) :: mLayerMatricHead(:,:)                 ! intent(in): [dp(:)] matric head in each soil layer (m)
    real(rkind),intent(in) :: localAquiferStorage(:)              ! intent(in): [dp]    aquifer storage for the local column (m)
    real(rkind),intent(in) :: basinAquiferStorage(:)              ! intent(in): [dp]    aquifer storage for the single basin (m)
    ! input: shortwave radiation fluxes
    real(rkind),intent(in) :: scalarCanopySunlitLAI(:)            ! intent(in): [dp] sunlit leaf area (-)
    real(rkind),intent(in) :: scalarCanopyShadedLAI(:)            ! intent(in): [dp] shaded leaf area (-)
    integer(i4b),intent(in),value :: scalarCanopySunlitPAR            ! intent(in): [dp] average absorbed par for sunlit leaves (w m-2)
    integer(i4b),intent(in),value :: scalarCanopyShadedPAR            ! intent(in): [dp] average absorbed par for shaded leaves (w m-2)
    integer(i4b),intent(in),value :: scalarCanopyAbsorbedSolar        ! intent(in): [dp] solar radiation absorbed by canopy (W m-2)
    integer(i4b),intent(in),value :: scalarGroundAbsorbedSolar        ! intent(in): [dp] solar radiation absorbed by ground (W m-2)
    ! output: fraction of wetted canopy area
    real(rkind),intent(inout) :: scalarCanopyWetFraction(:)                  ! intent(out): [dp] fraction of canopy that is wet
    ! output: longwave radiation fluxes
    real(rkind),intent(inout) :: scalarCanopyEmissivity(:)           ! intent(out): [dp] effective emissivity of the canopy (-)
    integer(i4b),intent(in),value :: scalarLWRadCanopy                ! intent(out): [dp] longwave radiation emitted from the canopy (W m-2)
    integer(i4b),intent(in),value :: scalarLWRadGround                ! intent(out): [dp] longwave radiation emitted at the ground surface (W m-2)
    integer(i4b),intent(in),value :: scalarLWRadUbound2Canopy         ! intent(out): [dp] downward atmospheric longwave radiation absorbed by the canopy (W m-2)
    integer(i4b),intent(in),value :: scalarLWRadUbound2Ground         ! intent(out): [dp] downward atmospheric longwave radiation absorbed by the ground (W m-2)
    integer(i4b),intent(in),value :: scalarLWRadUbound2Ubound         ! intent(out): [dp] atmospheric radiation reflected by the ground and lost thru upper boundary (W m-2)
    integer(i4b),intent(in),value :: scalarLWRadCanopy2Ubound         ! intent(out): [dp] longwave radiation emitted from canopy lost thru upper boundary (W m-2)
    integer(i4b),intent(in),value :: scalarLWRadCanopy2Ground         ! intent(out): [dp] longwave radiation emitted from canopy absorbed by the ground (W m-2)
    integer(i4b),intent(in),value :: scalarLWRadCanopy2Canopy         ! intent(out): [dp] canopy longwave reflected from ground and absorbed by the canopy (W m-2)
    integer(i4b),intent(in),value :: scalarLWRadGround2Ubound         ! intent(out): [dp] longwave radiation emitted from ground lost thru upper boundary (W m-2)
    integer(i4b),intent(in),value :: scalarLWRadGround2Canopy         ! intent(out): [dp] longwave radiation emitted from ground and absorbed by the canopy (W m-2)
    integer(i4b),intent(in),value :: scalarLWNetCanopy                ! intent(out): [dp] net longwave radiation at the canopy (W m-2)
    integer(i4b),intent(in),value :: scalarLWNetGround                ! intent(out): [dp] net longwave radiation at the ground surface (W m-2)
    integer(i4b),intent(in),value :: scalarLWNetUbound                ! intent(out): [dp] net longwave radiation at the upper boundary (W m-2)
    ! output: aerodynamic resistance
    real(rkind),intent(inout) :: scalarZ0Canopy(:)                   ! intent(out): [dp] roughness length of the canopy (m)
    real(rkind),intent(inout) :: scalarWindReductionFactor(:)        ! intent(out): [dp] canopy wind reduction factor (-)
    real(rkind),intent(inout) :: scalarZeroPlaneDisplacement(:)      ! intent(out): [dp] zero plane displacement (m)
    real(rkind),intent(inout) :: scalarRiBulkCanopy(:)               ! intent(out): [dp] bulk Richardson number for the canopy (-)
    real(rkind),intent(inout) :: scalarRiBulkGround(:)               ! intent(out): [dp] bulk Richardson number for the ground surface (-)
    integer(i4b),intent(in),value :: scalarEddyDiffusCanopyTop        ! intent(out): [dp] eddy diffusivity for heat at the top of the canopy (m2 s-1)
    integer(i4b),intent(in),value :: scalarFrictionVelocity           ! intent(out): [dp] friction velocity (m s-1)
    integer(i4b),intent(in),value :: scalarWindspdCanopyTop           ! intent(out): [dp] windspeed at the top of the canopy (m s-1)
    integer(i4b),intent(in),value :: scalarWindspdCanopyBottom        ! intent(out): [dp] windspeed at the height of the bottom of the canopy (m s-1)
    integer(i4b),intent(in),value :: scalarLeafResistance             ! intent(out): [dp] mean leaf boundary layer resistance per unit leaf area (s m-1)
    integer(i4b),intent(in),value :: scalarGroundResistance           ! intent(out): [dp] below canopy aerodynamic resistance (s m-1)
    integer(i4b),intent(in),value :: scalarCanopyResistance           ! intent(out): [dp] above canopy aerodynamic resistance (s m-1)
    ! input/output: soil resistance -- intent(in) and intent(inout) because only called at the first flux call
    real(rkind),intent(inout) :: mLayerRootDensity(:,:)                ! intent(in):    [dp] root density in each layer (-)
    real(rkind),intent(inout) :: scalarAquiferRootFrac(:)            ! intent(in):    [dp] fraction of roots below the lowest soil layer (-)
    real(rkind),intent(inout) :: scalarTranspireLim(:)               ! intent(inout): [dp] weighted average of the transpiration limiting factor (-)
    real(rkind),intent(inout) :: mLayerTranspireLim(:,:)               ! intent(inout): [dp] transpiration limiting factor in each layer (-)
    real(rkind),intent(inout) :: scalarTranspireLimAqfr(:)           ! intent(inout): [dp] transpiration limiting factor for the aquifer (-)
    real(rkind),intent(inout) :: scalarSoilRelHumidity(:)            ! intent(inout): [dp] relative humidity in the soil pores [0-1]
    integer(i4b),intent(in),value :: scalarSoilResistance             ! intent(inout): [dp] resistance from the soil (s m-1)
    ! input/output: stomatal resistance -- intent(inout) because only called at the first flux call
    integer(i4b),intent(in),value :: scalarStomResistSunlit           ! intent(inout): [dp] stomatal resistance for sunlit leaves (s m-1)
    integer(i4b),intent(in),value :: scalarStomResistShaded           ! intent(inout): [dp] stomatal resistance for shaded leaves (s m-1)
    integer(i4b),intent(in),value :: scalarPhotosynthesisSunlit       ! intent(inout): [dp] sunlit photosynthesis (umolco2 m-2 s-1)
    integer(i4b),intent(in),value :: scalarPhotosynthesisShaded       ! intent(inout): [dp] shaded photosynthesis (umolco2 m-2 s-1)
    ! output: turbulent heat fluxes
    real(rkind),intent(inout) :: scalarLatHeatSubVapCanopy(:)        ! intent(inout): [dp] latent heat of sublimation/vaporization for the vegetation canopy (J kg-1)
    real(rkind),intent(inout) :: scalarLatHeatSubVapGround(:)        ! intent(inout): [dp] latent heat of sublimation/vaporization for the ground surface (J kg-1)
    real(rkind),intent(inout) :: scalarSatVP_canopyTemp(:)           ! intent(out):   [dp] saturation vapor pressure at the temperature of the vegetation canopy (Pa)
    real(rkind),intent(inout) :: scalarSatVP_groundTemp(:)           ! intent(out):   [dp] saturation vapor pressure at the temperature of the ground surface (Pa)
    integer(i4b),intent(in),value :: scalarSenHeatTotal               ! intent(out):   [dp] sensible heat from the canopy air space to the atmosphere (W m-2)
    integer(i4b),intent(in),value :: scalarSenHeatCanopy              ! intent(out):   [dp] sensible heat flux from the canopy to the canopy air space (W m-2)
    integer(i4b),intent(in),value :: scalarSenHeatGround              ! intent(out):   [dp] sensible heat flux from ground surface below vegetation (W m-2)
    integer(i4b),intent(in),value :: scalarLatHeatTotal               ! intent(out):   [dp] latent heat from the canopy air space to the atmosphere (W m-2)
    integer(i4b),intent(in),value :: scalarLatHeatCanopyEvap          ! intent(out):   [dp] latent heat flux for evaporation from the canopy to the canopy air space (W m-2)
    integer(i4b),intent(in),value :: scalarLatHeatCanopyTrans         ! intent(out):   [dp] latent heat flux for transpiration from the canopy to the canopy air space (W m-2)
    integer(i4b),intent(in),value :: scalarLatHeatGround              ! intent(out):   [dp] latent heat flux from ground surface below vegetation (W m-2)
    ! output: advective heat fluxes
    integer(i4b),intent(in),value :: scalarCanopyAdvectiveHeatFlux    ! intent(out): [dp] heat advected to the canopy surface with rain + snow (W m-2)
    integer(i4b),intent(in),value :: scalarGroundAdvectiveHeatFlux    ! intent(out): [dp] heat advected to the ground surface with throughfall (W m-2)
    ! output: mass fluxes
    integer(i4b),intent(in),value :: scalarCanopySublimation          ! intent(out): [dp] canopy sublimation/frost (kg m-2 s-1)
    integer(i4b),intent(in),value :: scalarSnowSublimation            ! intent(out): [dp] snow sublimation/frost -- below canopy or non-vegetated (kg m-2 s-1)
    ! input/output: canopy air space variables
    real(rkind),intent(inout) :: scalarVP_CanopyAir(:)               ! intent(inout): [dp] vapor pressure of the canopy air space (Pa)
    real(rkind),intent(inout) :: scalarCanopyStabilityCorrection(:)  ! intent(inout): [dp] stability correction for the canopy (-)
    real(rkind),intent(inout) :: scalarGroundStabilityCorrection(:)  ! intent(inout): [dp] stability correction for the ground surface (-)
    ! output: liquid water fluxes
    integer(i4b),intent(in),value :: scalarCanopyTranspiration        ! intent(out): [dp] canopy transpiration (kg m-2 s-1)
    integer(i4b),intent(in),value :: scalarCanopyEvaporation          ! intent(out): [dp] canopy evaporation/condensation (kg m-2 s-1)
    integer(i4b),intent(in),value :: scalarGroundEvaporation          ! intent(out): [dp] ground evaporation/condensation -- below canopy or non-vegetated (kg m-2 s-1)
    ! output: derived fluxes
    integer(i4b),intent(in),value :: scalarTotalET                    ! intent(out): [dp] total ET (kg m-2 s-1)
    integer(i4b),intent(in),value :: scalarNetRadiation               ! intent(out): [dp] net radiation (W m-2)
    ! output: liquid water fluxes associated with evaporation/transpiration (needed for coupling)
    integer(i4b),intent(in),value :: returnCanopyTranspiration  ! intent(out): [dp] canopy transpiration (kg m-2 s-1)
    integer(i4b),intent(in),value :: returnCanopyEvaporation    ! intent(out): [dp] canopy evaporation/condensation (kg m-2 s-1)
    integer(i4b),intent(in),value :: returnGroundEvaporation    ! intent(out): [dp] ground evaporation/condensation -- below canopy or non-vegetated (kg m-2 s-1)
    ! output: fluxes
    real(rkind),intent(inout) :: flux_data(:,:)
    integer(i4b),intent(in),value :: canairNetFlux  ! intent(out): [dp] net energy flux for the canopy air space (W m-2)
    integer(i4b),intent(in),value :: canopyNetFlux  ! intent(out): [dp] net energy flux for the vegetation canopy (W m-2)
    integer(i4b),intent(in),value :: groundNetFlux  ! intent(out): [dp] net energy flux for the ground surface (W m-2)
    ! output: energy flux derivatives
    real(rkind),intent(inout) :: dCanairNetFlux_dCanairTemp(:) ! intent(out): [dp] derivative in net canopy air space flux w.r.t. canopy air temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dCanairNetFlux_dCanopyTemp(:) ! intent(out): [dp] derivative in net canopy air space flux w.r.t. canopy temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dCanairNetFlux_dGroundTemp(:) ! intent(out): [dp] derivative in net canopy air space flux w.r.t. ground temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dCanopyNetFlux_dCanairTemp(:) ! intent(out): [dp] derivative in net canopy flux w.r.t. canopy air temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dCanopyNetFlux_dCanopyTemp(:) ! intent(out): [dp] derivative in net canopy flux w.r.t. canopy temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dCanopyNetFlux_dGroundTemp(:) ! intent(out): [dp] derivative in net canopy flux w.r.t. ground temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dGroundNetFlux_dCanairTemp(:) ! intent(out): [dp] derivative in net ground flux w.r.t. canopy air temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dGroundNetFlux_dCanopyTemp(:) ! intent(out): [dp] derivative in net ground flux w.r.t. canopy temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dGroundNetFlux_dGroundTemp(:) ! intent(out): [dp] derivative in net ground flux w.r.t. ground temperature (W m-2 K-1)
    ! output: liquid flux derivatives (canopy evap)
    real(rkind),intent(inout) :: dCanopyEvaporation_dCanWat(:)   ! intent(out): [dp] derivative in canopy evaporation w.r.t. canopy total water content (s-1)
    real(rkind),intent(inout) :: dCanopyEvaporation_dTCanair(:)  ! intent(out): [dp] derivative in canopy evaporation w.r.t. canopy air temperature (kg m-2 s-1 K-1)
    real(rkind),intent(inout) :: dCanopyEvaporation_dTCanopy(:)  ! intent(out): [dp] derivative in canopy evaporation w.r.t. canopy temperature (kg m-2 s-1 K-1)
    real(rkind),intent(inout) :: dCanopyEvaporation_dTGround(:)  ! intent(out): [dp] derivative in canopy evaporation w.r.t. ground temperature (kg m-2 s-1 K-1)
    ! output: liquid flux derivatives (ground evap)
    real(rkind),intent(inout) :: dGroundEvaporation_dCanWat(:)   ! intent(out): [dp] derivative in ground evaporation w.r.t. canopy total water content (s-1)
    real(rkind),intent(inout) :: dGroundEvaporation_dTCanair(:)  ! intent(out): [dp] derivative in ground evaporation w.r.t. canopy air temperature (kg m-2 s-1 K-1)
    real(rkind),intent(inout) :: dGroundEvaporation_dTCanopy(:)  ! intent(out): [dp] derivative in ground evaporation w.r.t. canopy temperature (kg m-2 s-1 K-1)
    real(rkind),intent(inout) :: dGroundEvaporation_dTGround(:)  ! intent(out): [dp] derivative in ground evaporation w.r.t. ground temperature (kg m-2 s-1 K-1)
    ! output: transpiration derivatives
    real(rkind),intent(inout) :: dCanopyTrans_dCanWat(:)   ! intent(out): [dp] derivative in canopy transpiration w.r.t. canopy total water content (s-1)
    real(rkind),intent(inout) :: dCanopyTrans_dTCanair(:)  ! intent(out): [dp] derivative in canopy transpiration w.r.t. canopy air temperature (kg m-2 s-1 K-1)
    real(rkind),intent(inout) :: dCanopyTrans_dTCanopy(:)  ! intent(out): [dp] derivative in canopy transpiration w.r.t. canopy temperature (kg m-2 s-1 K-1)
    real(rkind),intent(inout) :: dCanopyTrans_dTGround(:)  ! intent(out): [dp] derivative in canopy transpiration w.r.t. ground temperature (kg m-2 s-1 K-1)
    ! output: cross derivative terms
    real(rkind),intent(inout) :: dCanopyNetFlux_dCanWat(:)  ! intent(out): [dp] derivative in net canopy fluxes w.r.t. canopy total water content (J kg-1 s-1)
    real(rkind),intent(inout) :: dGroundNetFlux_dCanWat(:)  ! intent(out): [dp] derivative in net ground fluxes w.r.t. canopy total water content (J kg-1 s-1)
    real(rkind),intent(inout) :: scalarIntercellularCO2Sunlit(:),scalarIntercellularCO2Shaded(:)
    real(rkind),intent(inout) :: iLayerThermalC(:,:)
    real(rkind),intent(in) :: mLayerDepth(:,:)
    real(rkind),intent(in) :: vcmax25_canopyTop(:),Kc25(:),Ko25(:),Kc_qFac(:),Ko_qFac(:),kc_Ha(:),ko_Ha(:),vcmax_qFac(:),vcmax_Ha(:),vcmax_Hd(:),vcmax_Sv(:),vcmax_Kn(:),jmax25_scale(:),jmax_Ha(:),jmax_Hd(:),jmax_Sv(:),fractionJ(:),quantamYield(:),vpScaleFactor(:),cond2photo_slope(:),minStomatalConductance(:)
 real(rkind),intent(inout) :: bp_noahmp(:),mp_noahmp(:),c3psn_noahmp(:),AVCMX_noahmp(:),vcmx25_noahmp(:),ako_noahmp(:),ko25_noahmp(:)
 real(rkind),intent(inout) :: akc_noahmp(:),kc25_noahmp(:),qe25_noahmp(:),folnmx_noahmp(:)
 real(rkind),intent(inout) :: rsmin_noahmp(:),hs_noahmp(:),topt_noahmp(:),rsmax_noahmp(:),rgl_noahmp(:)
real(rkind),intent(in),value :: minExpLogHgtFac
  ...
  if (ixCasNrg(iGRU)/=integerMissing .or. ixVegNrg(iGRU)/=integerMissing .or. ixTopNrg(iGRU)/=integerMissing) call vegNrgFlux_device(firstSubStep,firstFluxCall,computeVegFlux(iGRU),checkLWBalance,&
  upperBoundTemp(iGRU),canairTempTrial(iGRU),canopyTempTrial(iGRU),groundTempTrial(1,iGRU),canopyIceTrial(iGRU),canopyLiqTrial(iGRU),&
  dCanLiq_dTcanopy(iGRU),&
  ix_bcUpprTdyn,ix_veg_traits,ix_canopyEmis,ix_windPrfile,ix_astability,ix_soilStress,ix_groundwatr,ix_stomResist,ix_spatial_gw,&
  ix_bbTempFunc,ix_bbHumdFunc,ix_bbElecFunc,ix_bbCO2point,ix_bbNumerics,ix_bbAssimFnc,ix_bbCanIntg8, &
  nSnow(iGRU),nSoil,nLayers(iGRU),&
  vegTypeIndex(iGRU),soilTypeIndex(iGRU),&
  heightCanopyTop(iGRU),heightCanopyBottom(iGRU),canopyWettingFactor(iGRU),canopyWettingExp(iGRU),scalarCanopyIceMax(iGRU),scalarCanopyLiqMax(iGRU),&
scalarLAI(iGRU),scalarSAI(iGRU),scalarExposedLAI(iGRU),scalarExposedSAI(iGRU),scalarGrowingSeasonIndex(iGRU),scalarFoliageNitrogenFactor(iGRU),&
z0Snow(iGRU),z0Soil(iGRU),z0CanopyParam(iGRU),zpdFraction(iGRU),critRichNumber(iGRU),Louis79_bparam(iGRU),Louis79_cStar(iGRU),Mahrt87_eScale(iGRU),windReductionParam(iGRU),leafExchangeCoeff(iGRU),leafDimension(iGRU),&
theta_sat(1,iGRU),theta_res(1,iGRU),plantWiltPsi(iGRU),soilStressParam(iGRU),critSoilWilting(iGRU),critSoilTranspire(iGRU),critAquiferTranspire(iGRU),minStomatalResistance(iGRU),&
mHeight(iGRU),airtemp(iGRU),windspd(iGRU),airpres(iGRU),LWRadAtm(iGRU),scalarVPair(iGRU),scalarO2air(iGRU),scalarCO2air(iGRU),scalarTwetbulb(iGRU),flux_data(scalarRainfall,iGRU),flux_data(scalarSnowfall,iGRU),flux_data(scalarThroughfallRain,iGRU),flux_data(scalarThroughfallSnow,iGRU),&
scalarSWE(iGRU),scalarSnowDepth(iGRU),scalarGroundSnowFraction(iGRU),mLayerVolFracLiq(:,iGRU),mLayerMatricHead(:,iGRU),localAquiferStorage(iGRU),basinAquiferStorage(iGRU),&
scalarCanopySunlitLAI(iGRU),scalarCanopyShadedLAI(iGRU),flux_data(scalarCanopySunlitPAR,iGRU),flux_data(scalarCanopyShadedPAR,iGRU),flux_data(scalarCanopyAbsorbedSolar,iGRU),flux_data(scalarGroundAbsorbedSolar,iGRU),&
scalarCanopyWetFraction(iGRU), &
scalarCanopyEmissivity(iGRU),flux_data(scalarLWRadCanopy,iGRU),flux_data(scalarLWRadGround,iGRU),flux_data(scalarLWRadUbound2Canopy,iGRU),flux_data(scalarLWRadUbound2Ground,iGRU),flux_data(scalarLWRadUbound2Ubound,iGRU),flux_data(scalarLWRadCanopy2Ubound,iGRU),flux_data(scalarLWRadCanopy2Ground,iGRU),flux_data(scalarLWRadCanopy2Canopy,iGRU),flux_data(scalarLWRadGround2Ubound,iGRU),flux_data(scalarLWRadGround2Canopy,iGRU),flux_data(scalarLWNetCanopy,iGRU),flux_data(scalarLWNetGround,iGRU),flux_data(scalarLWNetUbound,iGRU),&
scalarZ0Canopy(iGRU),scalarWindReductionFactor(iGRU),scalarZeroPlaneDisplacement(iGRU),scalarRiBulkCanopy(iGRU),scalarRiBulkGround(iGRU),flux_data(scalarEddyDiffusCanopyTop,iGRU),flux_data(scalarFrictionVelocity,iGRU),flux_data(scalarWindspdCanopyTop,iGRU),flux_data(scalarWindspdCanopyBottom,iGRU),flux_data(scalarLeafResistance,iGRU),flux_data(scalarGroundResistance,iGRU),flux_data(scalarCanopyResistance,iGRU),&
mLayerRootDensity(:,iGRU),scalarAquiferRootFrac(iGRU),scalarTranspireLim(iGRU),mLayerTranspireLim(:,iGRU),scalarTranspireLimAqfr(iGRU),scalarSoilRelHumidity(iGRU),flux_data(scalarSoilResistance,iGRU),&
flux_data(scalarStomResistSunlit,iGRU),flux_data(scalarStomResistShaded,iGRU),flux_data(scalarPhotosynthesisSunlit,iGRU),flux_data(scalarPhotosynthesisShaded,iGRU),&
scalarLatHeatSubVapCanopy(iGRU),scalarLatHeatSubVapGround(iGRU),scalarSatVP_canopyTemp(iGRU),scalarSatVP_groundTemp(iGRU),flux_data(scalarSenHeatTotal,iGRU),flux_data(scalarSenHeatCanopy,iGRU),flux_data(scalarSenHeatGround,iGRU),flux_data(scalarLatHeatTotal,iGRU),flux_data(scalarLatHeatCanopyEvap,iGRU),flux_data(scalarLatHeatCanopyTrans,iGRU),flux_data(scalarLatHeatGround,iGRU),&
flux_data(scalarCanopyAdvectiveHeatFlux,iGRU),flux_data(scalarGroundAdvectiveHeatFlux,iGRU),&
flux_data(scalarCanopySublimation,iGRU),flux_data(scalarSnowSublimation,iGRU),&
scalarVP_CanopyAir(iGRU),scalarCanopyStabilityCorrection(iGRU),scalarGroundStabilityCorrection(iGRU),&
flux_data(scalarCanopyTranspiration,iGRU),flux_data(scalarCanopyEvaporation,iGRU),flux_data(scalarGroundEvaporation,iGRU),&
flux_data(scalarTotalET,iGRU),flux_data(scalarNetRadiation,iGRU),&
flux_data(returnCanopyTranspiration,iGRU),flux_data(returnCanopyEvaporation,iGRU),flux_data(returnGroundEvaporation,iGRU),&
flux_data(canairNetFlux,iGRU),flux_data(canopyNetFlux,iGRU),flux_data(groundNetFlux,iGRU),&
dCanairNetFlux_dCanairTemp(iGRU),dCanairNetFlux_dCanopyTemp(iGRU),dCanairNetFlux_dGroundTemp(iGRU),dCanopyNetFlux_dCanairTemp(iGRU),dCanopyNetFlux_dCanopyTemp(iGRU),dCanopyNetFlux_dGroundTemp(iGRU),dGroundNetFlux_dCanairTemp(iGRU),dGroundNetFlux_dCanopyTemp(iGRU),dGroundNetFlux_dGroundTemp(iGRU),&
dCanopyEvaporation_dCanWat(iGRU),dCanopyEvaporation_dTCanair(iGRU),dCanopyEvaporation_dTCanopy(iGRU),dCanopyEvaporation_dTGround(iGRU),&
dGroundEvaporation_dCanWat(iGRU),dGroundEvaporation_dTCanair(iGRU),dGroundEvaporation_dTCanopy(iGRU),dGroundEvaporation_dTGround(iGRU),&
dCanopyTrans_dCanWat(iGRU),dCanopyTrans_dTCanair(iGRU),dCanopyTrans_dTCanopy(iGRU),dCanopyTrans_dTGround(iGRU),&
dCanopyNetFlux_dCanWat(iGRU),dGroundNetFlux_dCanWat(iGRU),&
scalarIntercellularCO2Sunlit(iGRU),scalarIntercellularCO2Shaded(iGRU),iLayerThermalC(0,iGRU),mLayerDepth(1,iGRU),&
vcmax25_canopyTop(iGRU),Kc25(iGRU),Ko25(iGRU),Kc_qFac(iGRU),Ko_qFac(iGRU),kc_Ha(iGRU),ko_Ha(iGRU),vcmax_qFac(iGRU),vcmax_Ha(iGRU),vcmax_Hd(iGRU),vcmax_Sv(iGRU),vcmax_Kn(iGRU),jmax25_scale(iGRU),jmax_Ha(iGRU),jmax_Hd(iGRU),jmax_Sv(iGRU),fractionJ(iGRU),quantamYield(iGRU),vpScaleFactor(iGRU),cond2photo_slope(iGRU),minStomatalConductance(iGRU),&
bp_noahmp,mp_noahmp,c3psn_noahmp,avcmx_noahmp,vcmx25_noahmp,ako_noahmp,ko25_noahmp,akc_noahmp,kc25_noahmp,qe25_noahmp,folnmx_noahmp,&
                              rsmin_noahmp(iGRU),hs_noahmp(iGRU),topt_noahmp(iGRU),rsmax_noahmp(iGRU),rgl_noahmp(iGRU),minExpLogHgtFac)

end subroutine

attributes(device) subroutine vegNrgFlux_device(firstSubStep,firstFluxCall,computeVegFlux,checkLWBalance,&
  upperBoundTemp,canairTempTrial,canopyTempTrial,groundTempTrial,canopyIceTrial,canopyLiqTrial,&
  dCanLiq_dTcanopy,&
  ix_bcUpprTdyn,ix_veg_traits,ix_canopyEmis,ix_windPrfile,ix_astability,ix_soilStress,ix_groundwatr,ix_stomResist,ix_spatial_gw,&
  ix_bbTempFunc,ix_bbHumdFunc,ix_bbElecFunc,ix_bbCO2point,ix_bbNumerics,ix_bbAssimFnc,ix_bbCanIntg8, &
...
bp_noahmp,mp_noahmp,c3psn_noahmp,avcmx_noahmp,vcmx25_noahmp,ako_noahmp,ko25_noahmp,akc_noahmp,kc25_noahmp,qe25_noahmp,folnmx_noahmp,&
  ...
  implicit none
    ! input: model control
    logical(lgt),intent(in) :: firstSubStep                     ! intent(in): [dp] flag to indicate if we are processing the first sub-step
    logical(lgt),intent(in) :: firstFluxCall                    ! intent(in): [dp] flag to indicate if we are processing the first flux call
    logical(lgt),intent(in) :: computeVegFlux                   ! intent(in): [dp] flag to indicate if computing fluxes over vegetation
    logical(lgt),intent(in) :: checkLWBalance                   ! intent(in): [dp] flag to check longwave balance
    ! input: model state variables
    real(rkind),intent(inout) :: upperBoundTemp                   ! intent(in): [dp] temperature of the upper boundary (K) --> NOTE: use air temperature
    real(rkind),intent(inout) :: canairTempTrial                  ! intent(in): [dp] trial value of canopy air space temperature (K)
    real(rkind),intent(inout) :: canopyTempTrial                  ! intent(in): [dp] trial value of canopy temperature (K)
    ...
    ! input: layer geometry
    integer(i4b),intent(in) :: nSnow ! intent(in): [i4b] number of snow layers
    integer(i4b),intent(in) :: nSoil ! intent(in): [i4b] number of soil layers
    integer(i4b),intent(in) :: nLayers ! intent(in): [i4b] total number of layers
    ! input: physical attributes
    integer(i4b),intent(in) :: vegTypeIndex                     ! intent(in): [i4b] vegetation type index
    integer(i4b),intent(in) :: soilTypeIndex                    ! intent(in): [i4b] soil type index
    ...
    real(rkind),intent(in) :: scalarCO2air                     ! intent(in): [dp] atmospheric co2 concentration (Pa)
    real(rkind),intent(in) :: scalarTwetbulb                   ! intent(in): [dp] wetbulb temperature (K)
    real(rkind),intent(in) :: scalarRainfall                   ! intent(in): [dp] computed rainfall rate (kg m-2 s-1)
    real(rkind),intent(in) :: scalarSnowfall                   ! intent(in): [dp] computed snowfall rate (kg m-2 s-1)
    real(rkind),intent(in) :: scalarThroughfallRain            ! intent(in): [dp] rainfall through the vegetation canopy (kg m-2 s-1)
    real(rkind),intent(in) :: scalarThroughfallSnow            ! intent(in): [dp] snowfall through the vegetation canopy (kg m-2 s-1)
    ! input: water storage
    ! NOTE: soil stress only computed at the start of the substep (firstFluxCall=.true.)
    real(rkind),intent(in) :: scalarSWE                        ! intent(in): [dp]    snow water equivalent on the ground (kg m-2)
    real(rkind),intent(in) :: scalarSnowDepth                  ! intent(in): [dp]    snow depth on the ground surface (m)
    ...
    real(rkind),intent(in) :: scalarCanopySunlitLAI            ! intent(in): [dp] sunlit leaf area (-)
    real(rkind),intent(in) :: scalarCanopyShadedLAI            ! intent(in): [dp] shaded leaf area (-)
    real(rkind),intent(in) :: scalarCanopySunlitPAR            ! intent(in): [dp] average absorbed par for sunlit leaves (w m-2)
    real(rkind),intent(in) :: scalarCanopyShadedPAR            ! intent(in): [dp] average absorbed par for shaded leaves (w m-2)
    real(rkind),intent(in) :: scalarCanopyAbsorbedSolar        ! intent(in): [dp] solar radiation absorbed by canopy (W m-2)
    real(rkind),intent(in) :: scalarGroundAbsorbedSolar        ! intent(in): [dp] solar radiation absorbed by ground (W m-2)
    ! output: fraction of wetted canopy area
    real(rkind),intent(inout) :: scalarCanopyWetFraction                  ! intent(out): [dp] fraction of canopy that is wet
    ! output: longwave radiation fluxes
    real(rkind),intent(inout) :: scalarCanopyEmissivity           ! intent(out): [dp] effective emissivity of the canopy (-)
    real(rkind),intent(inout) :: scalarLWRadCanopy                ! intent(out): [dp] longwave radiation emitted from the canopy (W m-2)
    real(rkind),intent(inout) :: scalarLWRadGround                ! intent(out): [dp] longwave radiation emitted at the ground surface (W m-2)
    real(rkind),intent(inout) :: scalarLWRadUbound2Canopy         ! intent(out): [dp] downward atmospheric longwave radiation absorbed by the canopy (W m-2)
    real(rkind),intent(inout) :: scalarLWRadUbound2Ground         ! intent(out): [dp] downward atmospheric longwave radiation absorbed by the ground (W m-2)
    real(rkind),intent(inout) :: scalarLWRadUbound2Ubound         ! intent(out): [dp] atmospheric radiation reflected by the ground and lost thru upper boundary (W m-2)
    real(rkind),intent(inout) :: scalarLWRadCanopy2Ubound         ! intent(out): [dp] longwave radiation emitted from canopy lost thru upper boundary (W m-2)
    real(rkind),intent(inout) :: scalarLWRadCanopy2Ground         ! intent(out): [dp] longwave radiation emitted from canopy absorbed by the ground (W m-2)
    real(rkind),intent(inout) :: scalarLWRadCanopy2Canopy         ! intent(out): [dp] canopy longwave reflected from ground and absorbed by the canopy (W m-2)
    real(rkind),intent(inout) :: scalarLWRadGround2Ubound         ! intent(out): [dp] longwave radiation emitted from ground lost thru upper boundary (W m-2)
    real(rkind),intent(inout) :: scalarLWRadGround2Canopy         ! intent(out): [dp] longwave radiation emitted from ground and absorbed by the canopy (W m-2)
    real(rkind),intent(inout) :: scalarLWNetCanopy                ! intent(out): [dp] net longwave radiation at the canopy (W m-2)
    real(rkind),intent(inout) :: scalarLWNetGround                ! intent(out): [dp] net longwave radiation at the ground surface (W m-2)
    real(rkind),intent(inout) :: scalarLWNetUbound                ! intent(out): [dp] net longwave radiation at the upper boundary (W m-2)
    ! output: aerodynamic resistance
    real(rkind),intent(inout) :: scalarZ0Canopy                   ! intent(out): [dp] roughness length of the canopy (m)
    real(rkind),intent(inout) :: scalarWindReductionFactor        ! intent(out): [dp] canopy wind reduction factor (-)
    real(rkind),intent(inout) :: scalarZeroPlaneDisplacement      ! intent(out): [dp] zero plane displacement (m)
    real(rkind),intent(inout) :: scalarRiBulkCanopy               ! intent(out): [dp] bulk Richardson number for the canopy (-)
    real(rkind),intent(inout) :: scalarRiBulkGround               ! intent(out): [dp] bulk Richardson number for the ground surface (-)
    real(rkind),intent(inout) :: scalarEddyDiffusCanopyTop        ! intent(out): [dp] eddy diffusivity for heat at the top of the canopy (m2 s-1)
    real(rkind),intent(inout) :: scalarFrictionVelocity           ! intent(out): [dp] friction velocity (m s-1)
    real(rkind),intent(inout) :: scalarWindspdCanopyTop           ! intent(out): [dp] windspeed at the top of the canopy (m s-1)
    real(rkind),intent(inout) :: scalarWindspdCanopyBottom        ! intent(out): [dp] windspeed at the height of the bottom of the canopy (m s-1)
    real(rkind),intent(inout) :: scalarLeafResistance             ! intent(out): [dp] mean leaf boundary layer resistance per unit leaf area (s m-1)
    real(rkind),intent(inout) :: scalarGroundResistance           ! intent(out): [dp] below canopy aerodynamic resistance (s m-1)
    real(rkind),intent(inout) :: scalarCanopyResistance           ! intent(out): [dp] above canopy aerodynamic resistance (s m-1)
    ! input/output: soil resistance -- intent(in) and intent(inout) because only called at the first flux call
    real(rkind),intent(inout) :: mLayerRootDensity(:)                ! intent(in):    [dp] root density in each layer (-)
    real(rkind),intent(inout) :: scalarAquiferRootFrac            ! intent(in):    [dp] fraction of roots below the lowest soil layer (-)
    real(rkind),intent(inout) :: scalarTranspireLim               ! intent(inout): [dp] weighted average of the transpiration limiting factor (-)
    ...
    real(rkind),intent(inout) :: scalarTranspireLimAqfr           ! intent(inout): [dp] transpiration limiting factor for the aquifer (-)
    real(rkind),intent(inout) :: scalarSoilRelHumidity            ! intent(inout): [dp] relative humidity in the soil pores [0-1]
    real(rkind),intent(inout) :: scalarSoilResistance             ! intent(inout): [dp] resistance from the soil (s m-1)
    ! input/output: stomatal resistance -- intent(inout) because only called at the first flux call
    real(rkind),intent(inout) :: scalarStomResistSunlit           ! intent(inout): [dp] stomatal resistance for sunlit leaves (s m-1)
    real(rkind),intent(inout) :: scalarStomResistShaded           ! intent(inout): [dp] stomatal resistance for shaded leaves (s m-1)
    real(rkind),intent(inout) :: scalarPhotosynthesisSunlit       ! intent(inout): [dp] sunlit photosynthesis (umolco2 m-2 s-1)
    real(rkind),intent(inout) :: scalarPhotosynthesisShaded       ! intent(inout): [dp] shaded photosynthesis (umolco2 m-2 s-1)
    ! output: turbulent heat fluxes
    real(rkind),intent(inout) :: scalarLatHeatSubVapCanopy        ! intent(inout): [dp] latent heat of sublimation/vaporization for the vegetation canopy (J kg-1)
    real(rkind),intent(inout) :: scalarLatHeatSubVapGround        ! intent(inout): [dp] latent heat of sublimation/vaporization for the ground surface (J kg-1)
    real(rkind),intent(inout) :: scalarSatVP_canopyTemp           ! intent(out):   [dp] saturation vapor pressure at the temperature of the vegetation canopy (Pa)
    real(rkind),intent(inout) :: scalarSatVP_groundTemp           ! intent(out):   [dp] saturation vapor pressure at the temperature of the ground surface (Pa)
    real(rkind),intent(inout) :: scalarSenHeatTotal               ! intent(out):   [dp] sensible heat from the canopy air space to the atmosphere (W m-2)
    real(rkind),intent(inout) :: scalarSenHeatCanopy              ! intent(out):   [dp] sensible heat flux from the canopy to the canopy air space (W m-2)
    real(rkind),intent(inout) :: scalarSenHeatGround              ! intent(out):   [dp] sensible heat flux from ground surface below vegetation (W m-2)
    real(rkind),intent(inout) :: scalarLatHeatTotal               ! intent(out):   [dp] latent heat from the canopy air space to the atmosphere (W m-2)
    real(rkind),intent(inout) :: scalarLatHeatCanopyEvap          ! intent(out):   [dp] latent heat flux for evaporation from the canopy to the canopy air space (W m-2)
    real(rkind),intent(inout) :: scalarLatHeatCanopyTrans         ! intent(out):   [dp] latent heat flux for transpiration from the canopy to the canopy air space (W m-2)
    real(rkind),intent(inout) :: scalarLatHeatGround              ! intent(out):   [dp] latent heat flux from ground surface below vegetation (W m-2)
    ! output: advective heat fluxes
    real(rkind),intent(inout) :: scalarCanopyAdvectiveHeatFlux    ! intent(out): [dp] heat advected to the canopy surface with rain + snow (W m-2)
    real(rkind),intent(inout) :: scalarGroundAdvectiveHeatFlux    ! intent(out): [dp] heat advected to the ground surface with throughfall (W m-2)
    ! output: mass fluxes
    real(rkind),intent(inout) :: scalarCanopySublimation          ! intent(out): [dp] canopy sublimation/frost (kg m-2 s-1)
    real(rkind),intent(inout) :: scalarSnowSublimation            ! intent(out): [dp] snow sublimation/frost -- below canopy or non-vegetated (kg m-2 s-1)
    ! input/output: canopy air space variables
    real(rkind),intent(inout) :: scalarVP_CanopyAir               ! intent(inout): [dp] vapor pressure of the canopy air space (Pa)
    real(rkind),intent(inout) :: scalarCanopyStabilityCorrection  ! intent(inout): [dp] stability correction for the canopy (-)
    real(rkind),intent(inout) :: scalarGroundStabilityCorrection  ! intent(inout): [dp] stability correction for the ground surface (-)
    ! output: liquid water fluxes
    real(rkind),intent(inout) :: scalarCanopyTranspiration        ! intent(out): [dp] canopy transpiration (kg m-2 s-1)
    real(rkind),intent(inout) :: scalarCanopyEvaporation          ! intent(out): [dp] canopy evaporation/condensation (kg m-2 s-1)
    real(rkind),intent(inout) :: scalarGroundEvaporation          ! intent(out): [dp] ground evaporation/condensation -- below canopy or non-vegetated (kg m-2 s-1)
    ! output: derived fluxes
    real(rkind),intent(inout) :: scalarTotalET                    ! intent(out): [dp] total ET (kg m-2 s-1)
    real(rkind),intent(inout) :: scalarNetRadiation               ! intent(out): [dp] net radiation (W m-2)
    ! output: liquid water fluxes associated with evaporation/transpiration (needed for coupling)
    real(rkind),intent(inout) :: returnCanopyTranspiration  ! intent(out): [dp] canopy transpiration (kg m-2 s-1)
    real(rkind),intent(inout) :: returnCanopyEvaporation    ! intent(out): [dp] canopy evaporation/condensation (kg m-2 s-1)
    real(rkind),intent(inout) :: returnGroundEvaporation    ! intent(out): [dp] ground evaporation/condensation -- below canopy or non-vegetated (kg m-2 s-1)
    ! output: fluxes
    real(rkind),intent(inout) :: canairNetFlux  ! intent(out): [dp] net energy flux for the canopy air space (W m-2)
    real(rkind),intent(inout) :: canopyNetFlux  ! intent(out): [dp] net energy flux for the vegetation canopy (W m-2)
    real(rkind),intent(inout) :: groundNetFlux  ! intent(out): [dp] net energy flux for the ground surface (W m-2)
    ! output: energy flux derivatives
    real(rkind),intent(inout) :: dCanairNetFlux_dCanairTemp ! intent(out): [dp] derivative in net canopy air space flux w.r.t. canopy air temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dCanairNetFlux_dCanopyTemp ! intent(out): [dp] derivative in net canopy air space flux w.r.t. canopy temperature (W m-2 K-1)
    real(rkind),intent(inout) :: dCanairNetFlux_dGroundTemp ! intent(out): [dp] derivative in net canopy air space flux w.r.t. ground temperature (W m-2 K-1)
 ...
 real(rkind),intent(inout) :: akc_noahmp(:),kc25_noahmp(:),qe25_noahmp(:),folnmx_noahmp(:)
 real(rkind),intent(inout) :: rsmin_noahmp,hs_noahmp,topt_noahmp,rsmax_noahmp,rgl_noahmp
real(rkind),intent(inout) :: minExpLogHgtFac
      ! ---------------------------------------------------------------------------------------
  ! * local variables
  ! ---------------------------------------------------------------------------------------
  ! general)
      ...
      ! ************************************************
       case(prescribedTemp,zeroFlux)
        ...
        if (ix_bcUpprTdyn == prescribedTemp) then
          groundNetFlux = -iLayerThermalC*(groundTempTrial - upperBoundTemp)/(mLayerDepth*0.5_rkind)
        elseif (ix_bcUpprTdyn == zeroFlux) then
          groundNetFlux = 0._rkind
        else
          err=20; !message=trim(message)//'unable to identify upper boundary condition for thermodynamics: expect the case to be prescribedTemp or zeroFlux'; 
          return
        end if
      ...
      case(energyFlux)
        ...
        if (firstFluxCall) then
          ...
          if (nSnow > 0) then ! case when there is snow on the ground (EXCLUDE "snow without a layer" -- in this case, evaporate from the soil)
            if (groundTempTrial > Tfreeze) then; err=20; 
            ! message=trim(message)//'do not expect ground temperature > 0 when snow is on the ground'; 
            return; end if
        ...
        call aeroResist(&
                        ...
                        ! output: error control
                        err,minExpLogHgtFac                        ) ! intent(out): error control
        if (err/=0) then; 
        ! message=trim(message)//trim(cmessage); 
        return; end if
        ...
        if (firstFluxCall) then
          ...
          call stomResist(&
                          ...
                          ! type_data,                         & ! intent(in):    type of vegetation and soil
                          vegTypeIndex, &
                          airtemp,airpres,                         & ! intent(in):    model forcing data
                          minStomatalResistance,vcmax25_canopyTop,Kc25,Ko25,Kc_qFac,Ko_qFac,kc_Ha,ko_Ha,vcmax_qFac,vcmax_Ha,vcmax_Hd,vcmax_Sv,vcmax_Kn,jmax25_scale,jmax_Ha,jmax_Hd,jmax_Sv,fractionJ,quantamYield,vpScaleFactor,cond2photo_slope,minStomatalConductance,                         & ! intent(in):    model parameters
                          ix_stomResist,ix_bbTempFunc,ix_bbHumdFunc,ix_bbElecFunc,ix_bbCO2point,ix_bbNumerics,ix_bbAssimFnc,ix_bbCanIntg8,                   & ! intent(in):    model decisions
                          ! input-output: data structures
                          ! diag_data,                         & ! intent(inout): model diagnostic variables for a local HRU
                          scalarO2air,scalarCO2air,scalarExposedLAI,scalarGrowingSeasonIndex,scalarFoliageNitrogenFactor,scalarTranspireLim, &
                          scalarIntercellularCO2Sunlit,scalarIntercellularCO2Shaded, &
                          ! flux_data,                         & ! intent(inout): model fluxes for a local HRU
                          scalarCanopySunlitPAR,scalarCanopyShadedPAR,scalarLeafResistance, &
                          scalarStomResistSunlit,scalarStomResistShaded,scalarPhotosynthesisSunlit,scalarPhotosynthesisShaded, &
                          ! output: error control
                          err,&
                          bp_noahmp,mp_noahmp,c3psn_noahmp,avcmx_noahmp,vcmx25_noahmp,ako_noahmp,ko25_noahmp,akc_noahmp,kc25_noahmp,qe25_noahmp,folnmx_noahmp,&
                              rsmin_noahmp,hs_noahmp,topt_noahmp,rsmax_noahmp,rgl_noahmp                       ) ! intent(out):   error control
          if (err/=0) then; 
          ! message=trim(message)//trim(cmessage); 
          return; end if
...
! *******************************************************************************************************
attributes(device,host) subroutine wettedFrac(&
                      ! input
...
! *******************************************************************************************************
attributes(device,host) subroutine wetFraction(derDesire,smoothing,canopyLiq,canopyMax,canopyWettingFactor,canopyWettingExp,canopyWetFraction,canopyWetFractionDeriv)
  implicit none
  ! dummy variables
...
! *******************************************************************************************************
attributes(device,host) subroutine thetaSmoother(derDesire,canopyLiq,smoothTheta,smoothThetaDeriv)
  implicit none
  ! dummy variables
  logical(lgt),intent(in)    :: derDesire                   ! flag to denote if analytical derivatives are desired
  real(rkind),intent(in)     :: canopyLiq                   ! liquid water content (kg m-2)
...
! *******************************************************************************************************
attributes(device) subroutine longwaveBal(&
                      ! input: model control
                      computeVegFlux,                 & ! intent(in):  flag to compute fluxes over vegetation
                      checkLWBalance,                 & ! intent(in):  flag to check longwave balance
                      ! input: canopy and ground temperature
...
! *******************************************************************************************************
attributes(device) subroutine aeroResist(&
                      ! input: model control
                      computeVegFlux,                & ! intent(in):  logical flag to compute vegetation fluxes (.false. if veg buried by snow)
  ...
  if (mHeight < heightCanopyTop) then
    err=20; 
    ! message=trim(message)//'measurement height is below the top of the canopy'; 
    return
  end if
  ...
  if (computeVegFlux) then ! if vegetation is exposed
    ...
    select case(ixVegTraits)
      ...
      case default
        err=10; 
        ! message=trim(message)//"unknown parameterization for vegetation roughness length and displacement height"; 
        return
    ...
    if (mHeight < zeroPlaneDisplacement+z0Canopy) then; err=20; 
    ! message=trim(message)//'measurement height is below the displacement height'; 
    return; end if
    ...
    if(heightCanopyTopAboveSnow < referenceHeight)then; err=20; 
    ! message=trim(message)//'canopy top height above snow < reference height'; 
    return; end if 
    ...
    if (canopyResistance < 0._rkind) then; err=20; 
    ! message=trim(message)//'canopy resistance < 0'; 
    return; end if
    ...
    if (leafResistance < 0._rkind) then; err=20; 
    ! message=trim(message)//'leaf resistance < 0'; 
    return; end if
    ...
    if (ixWindProfile==exponential) then
      ...
      if(z0Ground > referenceHeight)then; err=20; 
      ! message=trim(message)//'ground roughness length > reference height'; 
      return; end if
    ...
    if (groundResistance < 0._rkind) then; err=20; 
    ! message=trim(message)//'ground resistance < 0 [vegetation is present]'; 
    return; end if
  ...
  ! * compute resistance for the case without a canopy (bare ground, or canopy completely buried with snow)
    ...
    if (mHeight < snowDepth+z0Ground) then; err=20; 
    ! message=trim(message)//'measurement height < snow depth + roughness length'; 
    return; end if
    ...
    groundResistance = groundResistanceNeutral/groundStabilityCorrection
    if (groundResistance < 0._rkind) then; err=20; 
    ! message=trim(message)//'ground resistance < 0 [no vegetation]'; 
    return; end if
...
! *******************************************************************************************************
attributes(device) subroutine soilResist(&
                      ! input (model decisions)
  ...
  do iLayer=1,size(mLayerMatricHead)
    ...
    select case(ixSoilResist)
      ...
      case default    ! check identified the option
        err=20; 
        ! message=trim(message)//'cannot identify option for soil resistance'; 
        return
    end select
...
! ********************************************************************************
attributes(device) subroutine turbFluxes(&
                      ! input: model control
                      computeVegFlux,                & ! intent(in): logical flag to compute vegetation fluxes (.false. if veg buried by snow)
                      ! input: above-canopy forcing data
                      airtemp,                       & ! intent(in): air temperature of the air above the vegetation canopy (K)
...
! *******************************************************************************************************
attributes(device) subroutine aStability(&
                      ! input: control
                      ixStability,                    & ! input:  choice of stability function
                      ! input: forcing data, diagnostic and state variables
  ...
  select case(ixStability)
    ...
    case default
      err=10; 
      ! message=trim(message)//"optionNotFound[stability correction]"; 
      return
  end select
  dStabilityCorrection_dAirTemp = dRiBulk_dAirTemp * dStabilityCorrection_dRich
  dStabilityCorrection_dSfcTemp = dRiBulk_dSfcTemp * dStabilityCorrection_dRich
...
! *******************************************************************************************************
attributes(device) subroutine bulkRichardson(&
                          ! input
                          airTemp,                    & ! input:  air temperature (K)
                          sfcTemp,                    & ! input:  surface temperature (K)
                          windspd_diff,               & ! input:  wind speed difference air to surface (m s-1)

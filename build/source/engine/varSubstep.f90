...
subroutine varSubstep(&
  ...
  USE getVectorz_module,only:varExtract_host                 ! extract variables from the state vector
  ...
  type(flux_mask_device) :: fluxMask
  ...
  type(flux_count_device) :: fluxCount
  ! input/output: data structures
  ...
  logical(lgt),device                       :: nrgFluxModified                        ! flag to denote that the energy fluxes were modified
  ! energy fluxes
...
type(dim3) :: blocks,threads
  ...
  globalVars: associate(&
    ...
    ! call allocate_device_flux_prev(flux_temp,indx_data%var(iLookINDEX%nSnow)%dat(1),indx_data%var(iLookINDEX%nSoil)%dat(1),nGRU,indx_data%var(iLookINDEX%nLayers)%dat(1))
    ...
    substeps: do
      ...
      call updateProg(nGRU,dtSubstep,nSnow,nSoil,nLayers,untappedMelt,stateVecTrial,stateVecPrime,                                    & ! input: states
      ...
      call update_sumBalance_kernel<<<blocks,threads>>>(nGRU,ixCasNrg,ixVegNrg,ixVegHyd,ixAqWat,&
    nSoil,nLayers,&
    ixSnowSoilNrg, ixSnowSoilHyd, ixSoilOnlyHyd, &
    nrgFluxModified, &
    dtSubstep, dt, &
    sumBalance, balance, &
    sumCanopyEvaporation, sumLatHeatCanopyEvap, sumSenHeatCanopy,&
    flux_temp%ixScalarCanopyEvaporation,flux_temp%ixScalarLatHeatCanopyEvap,flux_temp%ixScalarSenHeatCanopy,&
    flux_data%data,flux_temp%data, &
    sumSoilCompress, diag_data%scalarSoilCompress, &
    sumLayerCompress, diag_data%mLayerCompress,&
    fluxMask%data, fluxCount%data,flux_mean%data)
  ...
  call finalize_varSubstep_kernel<<<blocks,threads>>>(nGRU,flux_data%data,flux_mean%data,fluxMask%data,&
  flux_data%ixScalarCanopyEvaporation, flux_data%ixScalarLatHeatCanopyEvap, flux_data%ixScalarSenHeatCanopy, &
  sumCanopyEvaporation, sumLatHeatCanopyEvap, sumSenHeatCanopy, &
  dt,nSoil, nLayers,&
  diag_data%scalarSoilCompress, sumSoilCompress, &
  diag_data%mLayerCompress, sumLayerCompress, &
  ixSoilOnlyHyd,&
  sumBalance, &
  diag_data%balanceCasNrg, diag_data%balanceVegNrg, diag_data%balanceVegMass, diag_data%balanceAqMass,&
  diag_data%balanceLayerNrg, diag_data%balanceLayerMass,&
...
attributes(global) subroutine finalize_varSubstep_kernel(nGRU,&
  flux_data, flux_mean, fluxMask,&
  scalarCanopyEvaporation, scalarLatHeatCanopyEvap, scalarSenHeatCanopy, &
  sumCanopyEvaporation, sumLatHeatCanopyEvap, sumSenHeatCanopy, &
  dt,nSoil, nLayers,&
  scalarSoilCompress, sumSoilCompress, &
  mLayerCompress, sumLayerCompress, &
  ixSoilOnlyHyd,&
  sumBalance, &
  balanceCasNrg, balanceVegNrg, balanceVegMass, balanceAqMass,&
  balanceLayerNrg, balanceLayerMass,&
  ixCasNrg, ixVegNrg, ixVegHyd, ixAqWat, &
  ixSnowSoilNrg, ixSnowSoilHyd)
  integer(i4b),intent(in),value :: nGRU
  real(rkind),intent(inout) :: flux_data(:,:), flux_mean(:,:)
  logical(lgt),intent(in) :: fluxMask(:,:)
  integer(i4b),intent(in),value :: scalarCanopyEvaporation, scalarLatHeatCanopyEvap, scalarSenHeatCanopy
  real(rkind),intent(inout) :: sumCanopyEvaporation(:), sumLatHeatCanopyEvap(:), sumSenHeatCanopy(:)
  real(rkind),intent(in),value :: dt
  integer(i4b),intent(in) :: nLayers(:)
  integer(i4b),intent(in),value :: nSoil
  real(rkind),intent(inout) :: scalarSoilCompress(:), sumSoilCompress(:)
  real(rkind),intent(inout) :: mLayerCompress(:,:), sumLayerCompress(:,:)
  integer(i4b),intent(in) :: ixSoilOnlyHyd(:,:)
  real(rkind),intent(inout) :: sumBalance(:,:)
  ...
  call finalize_varSubstep(fluxMask(:,iGRU), flux_data(:,iGRU), flux_mean(:,iGRU),&
  flux_data(scalarCanopyEvaporation,iGRU), flux_data(scalarLatHeatCanopyEvap,iGRU), flux_data(scalarSenHeatCanopy,iGRU),&
  sumCanopyEvaporation(iGRU), sumLatHeatCanopyEvap(iGRU), sumSenHeatCanopy(iGRU),&
  dt,nSoil,nLayers(iGRU), &
  scalarSoilCompress(iGRU), sumSoilCompress(iGRU), &
  mLayerCompress(:,iGRU), sumLayerCompress(:,iGRU), &
  ixSoilOnlyHyd(:,iGRU),&
  sumBalance(:,iGRU), &
  balanceCasNrg(iGRU), balanceVegNrg(iGRU), balanceVegMass(iGRU), balanceAqMass(iGRU),&
  balanceLayerNrg(:,iGRU), balanceLayerMass(:,iGRU),&
  ixCasNrg(iGRU), ixVegNrg(iGRU), ixVegHyd(iGRU), ixAqWat(iGRU), &
  ixSnowSoilNrg(:,iGRU), ixSnowSoilHyd(:,iGRU))
end subroutine
attributes(device) subroutine finalize_varSubstep(fluxMask, flux_data, flux_mean,&
  scalarCanopyEvaporation, scalarLatHeatCanopyEvap, scalarSenHeatCanopy, &
  sumCanopyEvaporation, sumLatHeatCanopyEvap, sumSenHeatCanopy,&
  dt,nSoil, nLayers, &
  scalarSoilCompress, sumSoilCompress, &
  ...
  real(rkind),intent(inout) :: flux_data(:), flux_mean(:)
  logical(lgt),intent(in) :: fluxMask(:)
  real(rkind),intent(inout) :: scalarCanopyEvaporation, scalarLatHeatCanopyEvap, scalarSenHeatCanopy
  real(rkind),intent(in) :: sumCanopyEvaporation, sumLatHeatCanopyEvap, sumSenHeatCanopy
  real(rkind),intent(in) :: dt
  integer(i4b),intent(in) :: nSoil, nLayers
  real(rkind),intent(inout) :: scalarSoilCompress, sumSoilCompress
  real(rkind),intent(inout) :: mLayerCompress(:), sumLayerCompress(:)
  integer(i4b),intent(in) :: ixSoilOnlyHyd(:)
  real(rkind),intent(inout) :: sumBalance(:)
  ...
  integer(i4b) :: iVar, iSoil, ixLayer
    ! save the fluxes as averages
    do iVar=1,size(fluxMask)
      if(fluxMask(iVar)) flux_data(iVar) = flux_mean(iVar)
    enddo

    ! save the energy fluxes as averages
    scalarCanopyEvaporation = sumCanopyEvaporation /dt      ! canopy evaporation/condensation (kg m-2 s-1)
    scalarLatHeatCanopyEvap = sumLatHeatCanopyEvap /dt      ! latent heat flux for evaporation from the canopy to the canopy air space (W m-2)
    scalarSenHeatCanopy     = sumSenHeatCanopy     /dt      ! sensible heat flux from the canopy to the canopy air space (W m-2)

    ! save the soil compression diagnostics as averages
    scalarSoilCompress = sumSoilCompress/dt
    do iSoil=1,nSoil
      if(ixSoilOnlyHyd(iSoil)/=integerMissing)&
      mLayerCompress(iSoil) = sumLayerCompress(iSoil)/dt
    end do

        ! save the balance diagnostics as averages
    if(ixCasNrg/=integerMissing) balanceCasNrg = sumBalance(ixCasNrg)/dt
    if(ixVegNrg/=integerMissing) balanceVegNrg = sumBalance(ixVegNrg)/dt
      do ixLayer=1,nLayers
        if (ixSnowSoilNrg(ixLayer)/=integerMissing) balanceLayerNrg(ixLayer) = sumBalance(ixSnowSoilNrg(ixLayer))/dt
      end do
    if(ixVegHyd/=integerMissing) balanceVegMass = sumBalance(ixVegHyd)/dt
      do ixLayer=1,nLayers
        if (ixSnowSoilHyd(ixLayer)/=integerMissing) balanceLayerMass(ixLayer) = sumBalance(ixSnowSoilHyd(ixLayer))/dt
      end do
    if(ixAqWat/=integerMissing) balanceAqMass = sumBalance(ixAqWat)/dt
...
! **********************************************************************************************************
subroutine updateProg(nGRU, dt,nSnow,nSoil,nLayers,untappedMelt,stateVecTrial,stateVecPrime,                                           & ! input: states
                      doAdjustTemp,computeVegFlux,computMassBalance,computNrgBalance,computeEnthTemp,enthalpyStateVec,use_lookup,& ! input: model control
                      model_decisions,lookup_data,mpar_data,indx_data,flux_data,prog_data,diag_data,deriv_data,                  & ! input-output: data structures
                      fluxVec,resVec,balance,waterBalanceError,nrgFluxModified,err,message)                                        ! input-output: balances, flags, and error control
USE getVectorz_module,only:varExtract_host,varExtract                              ! extract variables from the state vector
use device_data_types
  ...
  real(rkind)      ,intent(in),device    :: untappedMelt(:,:)               ! un-tapped melt energy (J m-3 s-1)
  real(rkind)      ,intent(in),device    :: stateVecTrial(:,:)              ! trial state vector (mixed units)
  real(rkind)      ,intent(in),device    :: stateVecPrime(:,:)              ! trial state vector (mixed units)
  ...
  real(rkind)      ,intent(inout),device :: balance(:,:)                    ! balance of energy per domain per second
  logical(lgt)     ,intent(out)   :: waterBalanceError             ! flag to denote that there is a water balance error
  logical(lgt)     ,intent(out),device   :: nrgFluxModified               ! flag to denote that the energy fluxes were modified
  ...
  ! mass balance
  real(rkind),device                     :: canopyBalance0(nGRU) ! canopy storage at start/end of time step
  real(rkind),device                     :: soilBalance0(nGRU),soilBalance1     ! soil storage at start/end of time step
  ...
  real(rkind),device                     :: scalarCanairTempTrial(nGRU)         ! trial value for temperature of the canopy air space (K)
  real(rkind),device                     :: scalarCanopyTempTrial(nGRU)         ! trial value for temperature of the vegetation canopy (K)
  real(rkind),device                     :: scalarCanopyWatTrial(nGRU)          ! trial value for liquid water storage in the canopy (kg m-2)
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerTempTrial               ! trial vector of temperature of layers in the snow and soil domains (K)
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerVolFracWatTrial         ! trial vector of volumetric fraction of total water (-)
  ...
  real(rkind),device                     :: scalarAquiferStorageTrial(nGRU)     ! trial value for storage of water in the aquifer (m)
  real(rkind),device                     :: scalarCanairEnthalpyTrial(nGRU)     ! trial value for enthalpy of the canopy air space (J m-3)
  real(rkind),device                     :: scalarCanopyEnthTempTrial(nGRU)     ! trial value for temperature component of enthalpy of the vegetation canopy (J m-3)
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerEnthTempTrial           ! trial vector of temperature component of enthalpy of snow + soil (J m-3)
  real(rkind),device                     :: scalarCanopyEnthalpyTrial(nGRU)     ! trial value for enthalpy of the vegetation canopy (J m-3)
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerEnthalpyTrial           ! trial vector of enthalpy of each snow and soil layer (J m-3)
  ! diagnostic variables
  real(rkind),device                     :: scalarCanopyLiqTrial(nGRU)          ! trial value for mass of liquid water on the vegetation canopy (kg m-2)
  real(rkind),device                     :: scalarCanopyIceTrial(nGRU)          ! trial value for mass of ice on the vegetation canopy (kg m-2)
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerVolFracLiqTrial         ! trial vector of volumetric fraction of liquid water (-)
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerVolFracIceTrial         ! trial vector of volumetric fraction of ice (-)
  ! prime state variables
  real(rkind),device                     :: scalarCanairTempPrime(nGRU)         ! trial value for temperature of the canopy air space (K)
  real(rkind),device                     :: scalarCanopyTempPrime(nGRU)         ! trial value for temperature of the vegetation canopy (K)
  real(rkind),device                     :: scalarCanopyWatPrime(nGRU)          ! trial value for liquid water storage in the canopy (kg m-2)
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerTempPrime               ! trial vector of temperature of layers in the snow and soil domains (K)
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerVolFracWatPrime         ! trial vector of volumetric fraction of total water (-)
  ...
  real(rkind),device                     :: scalarAquiferStoragePrime(nGRU)     ! trial value for storage of water in the aquifer (m)
  ! diagnostic prime or delta variables
  real(rkind),device                     :: scalarCanopyLiqPrime(nGRU)          ! trial value for mass of liquid water on the vegetation canopy (kg m-2)
  real(rkind),device                     :: scalarCanopyIcePrime(nGRU)          ! trial value for mass of ice on the vegetation canopy (kg m-2)
  ...
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerVolFracLiqPrime         ! trial vector of volumetric fraction of liquid water (-)
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerVolFracIcePrime         ! trial vector of volumetric fraction of ice (-)
  ...
  real(rkind),device                     :: scalarCanairNrgTrial(nGRU)        ! trial value for energy of the canopy air space
  real(rkind),device                     :: scalarCanopyNrgTrial(nGRU)        ! trial value for energy of the vegetation canopy
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerNrgTrial              ! trial vector of energy of each snow and soil layer
  real(rkind),device                     :: scalarCanairNrgPrime(nGRU)        ! prime value for energy of the canopy air space
  real(rkind),device                     :: scalarCanopyNrgPrime(nGRU)        ! prime value for energy of the vegetation canopy
  real(rkind),device,dimension(nSoil+maxSnowLayers+1,nGRU)  :: mLayerNrgPrime              ! prime vector of energy of each snow and soil layer
...
type(dim3) :: blocks,threads
  ...
  associate(&
    ...
    scalarThroughfallRain     => flux_data%ixScalarThroughfallRain      ,& ! intent(in)   : [dp]     rain reaches ground without touching the canopy (kg m-2 s-1)
    scalarCanopyEvaporation   => flux_data%ixScalarCanopyEvaporation    ,& ! intent(in)   : [dp]     canopy evaporation/condensation (kg m-2 s-1)
    scalarCanopyLiqDrainage   => flux_data%ixScalarCanopyLiqDrainage    ,& ! intent(in)   : [dp]     drainage liquid water from vegetation canopy (kg m-2 s-1)
    ! iLayerLiqFluxSoil         => flux_data%ixiLayerLiqFluxSoil             ,& ! intent(in)   : [dp(0:)] vertical liquid water flux at soil layer interfaces (-)
    ! iLayerNrgFlux             => flux_data%ixiLayerNrgFlux                 ,& ! intent(in)   :
    ! mLayerNrgFlux             => flux_data%ixmLayerNrgFlux                 ,& ! intent(out)  : [dp]     net energy flux for each layer within the snow+soil domain (J m-3 s-1)
    ! mLayerTranspire           => flux_data%ixmLayerTranspire               ,& ! intent(in)   : [dp(:)]  transpiration loss from each soil layer (m s-1)
    ! mLayerBaseflow            => flux_data%ixmLayerBaseflow                ,& ! intent(in)   : [dp(:)]  baseflow from each soil layer (m s-1)
    ! mLayerCompress            => diag_data%ixmLayerCompress                ,& ! intent(in)   : [dp(:)]  change in storage associated with compression of the soil matrix (-)
    ! energy fluxes
    scalarLatHeatCanopyEvap   => flux_data%ixScalarLatHeatCanopyEvap    ,& ! intent(in)   : [dp]     latent heat flux for evaporation from the canopy to the canopy air space (W m-2)
    scalarSenHeatCanopy       => flux_data%ixScalarSenHeatCanopy        ,& ! intent(in)   : [dp]     sensible heat flux from the canopy to the canopy air space (W m-2)
    ! domain depth
    ...
    call initialBalance_kernel<<<blocks,threads>>>(nGRU,canopyBalance0,soilBalance0, &
  scalarCanopyLiq, scalarCanopyIce, &
  computeVegFlux,&
  mLayerVolFracLiq, mLayerVolFracIce, mLayerDepth, &
  nSnow, indx_data%nLayers_d)
    ! -----
    ! * update states...
    ! ------------------
    ...
    call checkBalances_kernel<<<blocks,threads>>>(nGRU,computNrgBalance,computMassBalance, &
  ixCasNrg, ixVegNrg, ixVegHyd, ixAqWat, &
  indx_data%nLayers_d, nSnow, &
  ixSnowSoilNrg, ixSnowSoilHyd,ixDomainType,ixNrgOnly,ixControlVolume,&
  mLayerVolFracLiqTrial, mLayerVolFracIceTrial, &
  scalarCanopyLiqTrial, scalarCanopyIceTrial, scalarCanopyWatTrial, &
  balance, untappedMelt,&
  scalarRainfall, scalarCanopyEvaporation, scalarThroughfallRain, scalarCanopyLiqDrainage, &
  dt, canopyBalance0, &
  mpar_data%absTolWatVeg, mpar_data%absTolWatSnow,&
  canopyDepth,scalarLatHeatCanopyEvap, scalarSenHeatCanopy,&
  flux_data%data, &
  nrgFluxModified)
...
attributes(global) subroutine initialBalance_kernel(nGRU,canopyBalance0,soilBalance0, &
  scalarCanopyLiq, scalarCanopyIce, &
  computeVegFlux,&
  mLayerVolFracLiq, mLayerVolFracIce, mLayerDepth, &
  nSnow, nLayers)
  integer(i4b),intent(in),value :: nGRU
  real(rkind),intent(inout) :: canopyBalance0(:),soilBalance0(:)
  real(rkind),intent(in) :: scalarCanopyLiq(:), scalarCanopyIce(:)
  logical(lgt),intent(in) :: computeVegFlux(:)
  real(rkind),intent(in) :: mLayerVolFracLiq(:,:), mLayerVolFracIce(:,:), mLayerDepth(:,:)
  ...
  if (iGRU .gt. nGRU) return
  call initialBalance(canopyBalance0(iGRU),soilBalance0(iGRU), &
  scalarCanopyLiq(iGRU), scalarCanopyIce(iGRU), &
  computeVegFlux(iGRU),&
  mLayerVolFracLiq(:,iGRU), mLayerVolFracIce(:,iGRU), mLayerDepth(:,iGRU), &
  nSnow(iGRU), nLayers(iGRU))
end subroutine
attributes(device) subroutine initialBalance(canopyBalance0,soilBalance0, &
  scalarCanopyLiq, scalarCanopyIce, &
  computeVegFlux,&
  mLayerVolFracLiq, mLayerVolFracIce, mLayerDepth, &
  nSnow, nLayers)
...
attributes(global) subroutine checkBalances_kernel(nGRU,computNrgBalance,computMassBalance, &
  ixCasNrg, ixVegNrg, ixVegHyd, ixAqWat, &
  nLayers, nSnow, &
  ixSnowSoilNrg, ixSnowSoilHyd,ixDomainType,ixNrgOnly,ixControlVolume,&
  mLayerVolFracLiqTrial, mLayerVolFracIceTrial, &
  ...
  integer(i4b),intent(in) :: ixSnowSoilNrg(:,:), ixSnowSoilHyd(:,:), ixDomainType(:,:), ixNrgOnly(:,:), ixControlVolume(:,:)
  real(rkind),intent(inout) :: mLayerVolFracLiqTrial(:,:), mLayerVolFracIceTrial(:,:)
  real(rkind) :: scalarCanopyLiqTrial(:), scalarCanopyIceTrial(:), scalarCanopyWatTrial(:)
  real(rkind),intent(inout) :: balance(:,:), untappedMelt(:,:)
  integer(i4b),intent(in),value :: scalarRainfall, scalarCanopyEvaporation, scalarThroughfallRain, scalarCanopyLiqDrainage
  real(rkind),intent(in),value :: dt
  real(rkind),intent(inout) :: canopyBalance0(:)
  real(rkind),intent(inout) :: absTolWatVeg, absTolWatSnow
  real(rkind),intent(inout) :: canopyDepth(:), flux_data(:,:)
  integer(i4b),intent(in),value :: scalarLatHeatCanopyEvap, scalarSenHeatCanopy
  logical(lgt),intent(inout) :: nrgFluxModified
  ...
  call checkBalances(computNrgBalance,computMassBalance, &
  ixCasNrg(iGRU), ixVegNrg(iGRU), ixVegHyd(iGRU), ixAqWat(iGRU), &
  nLayers(iGRU), nSnow(iGRU), &
  ixSnowSoilNrg(:,iGRU), ixSnowSoilHyd(:,iGRU),ixDomainType(:,iGRU),ixNrgOnly(:,iGRU),ixControlVolume(:,iGRU),&
  mLayerVolFracLiqTrial(:,iGRU), mLayerVolFracIceTrial(:,iGRU), &
  scalarCanopyLiqTrial(iGRU), scalarCanopyIceTrial(iGRU), scalarCanopyWatTrial(iGRU), &
  balance(:,iGRU), untappedMelt(:,iGRU),&
  flux_data(scalarRainfall,iGRU), flux_data(scalarCanopyEvaporation,iGRU), flux_data(scalarThroughfallRain,iGRU), flux_data(scalarCanopyLiqDrainage,iGRU), &
  dt, canopyBalance0(iGRU), &
  absTolWatVeg, absTolWatSnow,&
  canopyDepth(iGRU),flux_data(scalarLatHeatCanopyEvap,iGRU), flux_data(scalarSenHeatCanopy,iGRU),&
  nrgFluxModified)
end subroutine


attributes(device) subroutine checkBalances(computNrgBalance,computMassBalance, &
  ixCasNrg, ixVegNrg, ixVegHyd, ixAqWat, &
  nLayers, nSnow, &
  ixSnowSoilNrg, ixSnowSoilHyd,ixDomainType,ixNrgOnly,ixControlVolume,&
  mLayerVolFracLiqTrial, mLayerVolFracIceTrial, &
  ...
  integer(i4b),intent(in) :: ixSnowSoilNrg(:), ixSnowSoilHyd(:), ixDomainType(:), ixNrgOnly(:), ixControlVolume(:)
  real(rkind),intent(inout) :: mLayerVolFracLiqTrial(:), mLayerVolFracIceTrial(:)
  real(rkind),intent(inout) :: scalarCanopyLiqTrial, scalarCanopyIceTrial, scalarCanopyWatTrial
  real(rkind),intent(inout) :: balance(:), untappedMelt(:)
  real(rkind),intent(inout) :: scalarRainfall, scalarCanopyEvaporation, scalarThroughfallRain, scalarCanopyLiqDrainage
  real(rkind),intent(inout) :: dt, canopyBalance0
  real(rkind),intent(inout) :: absTolWatVeg, absTolWatSnow
  real(rkind),intent(inout) :: canopyDepth, scalarLatHeatCanopyEvap, scalarSenHeatCanopy
  logical(lgt),intent(inout) :: nrgFluxModified

  real(rkind) :: superflousNrg, superflousWat, volMelt
  
  
  real(rkind) :: eps_veg, eps_snow
  real(rkind) :: canopyBalance1
  ...
  integer(i4b) :: err
        ...
        eps_veg = absTolWatVeg*2._rkind
        eps_snow = absTolWatSnow*2._rkind
    ...
    if(computNrgBalance)then
      ...
      if(ixVegNrg/=integerMissing) balance(ixVegNrg) = realMissing
        do i=1,nLayers
          if (ixSnowSoilNrg(i)/=integerMissing) balance(ixSnowSoilNrg(i)) = realMissing
        enddo
    ...
    if(computMassBalance)then
      ...
      if(ixVegHyd/=integerMissing) balance(ixVegHyd) = realMissing
        do i=1,nLayers
          if (ixSnowSoilHyd(i)/=integerMissing) balance(ixSnowSoilHyd(i)) = realMissing
        end do
    ...
    if(size(ixNrgOnly)>0)then  ! energy state variables exist
      ...
      do iState=1,size(ixNrgOnly)
        ...
        ixFullVector   = ixSubset    ! index within full state vector
  ...
  attributes(global) subroutine update_sumBalance_kernel(nGRU,ixCasNrg,ixVegNrg,ixVegHyd,ixAqWat,&
    nSoil,nLayers,&
    ixSnowSoilNrg, ixSnowSoilHyd, ixSoilOnlyHyd, &
    nrgFluxModified, &
    dtSubstep, dt, &
    ...
    scalarCanopyEvaporation,scalarLatHeatCanopyEvap,scalarSenHeatCanopy,&
    flux_data,flux_temp, &
    sumSoilCompress, scalarSoilCompress, &
    sumLayerCompress, mLayerCompress,&
    fluxMask, fluxCount,flux_mean)
  integer(i4b),intent(in),value :: nGRU
      integer(i4b),intent(in) :: ixCasNrg(:),ixVegNrg(:),ixVegHyd(:),ixAqWat(:)
      integer(i4b),intent(in),value :: nSoil
    integer(i4b),intent(in) :: nLayers(:)
    integer(i4b),intent(in) :: ixSnowSoilNrg(:,:), ixSnowSoilHyd(:,:), ixSoilOnlyHyd(:,:)
    logical(lgt),intent(in) :: nrgFluxModified
    real(rkind),intent(inout) :: sumBalance(:,:), balance(:,:)
    real(rkind),intent(in),value :: dtSubstep, dt
    real(rkind),intent(inout) :: sumCanopyEvaporation(:), sumLatHeatCanopyEvap(:), sumSenHeatCanopy(:)
    real(rkind),intent(inout) :: flux_data(:,:), flux_temp(:,:)
    integer(i4b),intent(in),value :: scalarCanopyEvaporation,scalarLatHeatCanopyEvap,scalarSenHeatCanopy
    real(rkind),intent(inout) :: sumSoilCompress(:), scalarSoilCompress(:)
    real(rkind),intent(inout) :: sumLayerCompress(:,:), mLayerCompress(:,:)
    logical(lgt),intent(in) :: fluxMask(:,:)
    integer(i4b),intent(inout) :: fluxCount(:,:)
  ...
  if (iGRU .gt. nGRU) return
  call update_sumBalance(ixCasNrg(iGRU),ixVegNrg(iGRU),ixVegHyd(iGRU),ixAqWat(iGRU),&
    nSoil,nLayers(iGRU),&
    ixSnowSoilNrg(:,iGRU), ixSnowSoilHyd(:,iGRU), ixSoilOnlyHyd(:,iGRU), &
    nrgFluxModified, &
    dtSubstep, dt, &
    sumBalance(:,iGRU), balance(:,iGRU), &
    sumCanopyEvaporation(iGRU), sumLatHeatCanopyEvap(iGRU), sumSenHeatCanopy(iGRU),&
    flux_temp(scalarCanopyEvaporation,iGRU),flux_temp(scalarLatHeatCanopyEvap,iGRU),flux_temp(scalarSenHeatCanopy,iGRU),&
    flux_data(scalarCanopyEvaporation,iGRU),flux_data(scalarLatHeatCanopyEvap,iGRU),flux_data(scalarSenHeatCanopy,iGRU), &
    sumSoilCompress(iGRU), scalarSoilCompress(iGRU), &
    sumLayerCompress(:,iGRU), mLayerCompress(:,iGRU), &
    fluxMask(:,iGRU),fluxCount(:,iGRU), flux_mean(:,iGRU), flux_temp(:,iGRU))
  end subroutine
  attributes(device) subroutine update_sumBalance(ixCasNrg,ixVegNrg,ixVegHyd,ixAqWat,&
    nSoil,nLayers,&
    ixSnowSoilNrg, ixSnowSoilHyd, ixSoilOnlyHyd, &
    nrgFluxModified, &
    dtSubstep, dt, &
    sumBalance, balance, &
    sumCanopyEvaporation, sumLatHeatCanopyEvap, sumSenHeatCanopy,&
    scalarCanopyEvaporation_temp,scalarLatHeatCanopyEvap_temp,scalarSenHeatCanopy_temp,&
    scalarCanopyEvaporation_data,scalarLatHeatCanopyEvap_data,scalarSenHeatCanopy_data, &
    sumSoilCompress, scalarSoilCompress, &
    sumLayerCompress, mLayerCompress, &
    fluxMask, fluxCount, flux_mean,flux_temp)
    integer(i4b),intent(in) :: ixCasNrg,ixVegNrg,ixVegHyd,ixAqWat
    integer(i4b),intent(in) :: nSoil, nLayers
    integer(i4b),intent(in) :: ixSnowSoilNrg(:), ixSnowSoilHyd(:), ixSoilOnlyHyd(:)
    logical(lgt),intent(in) :: nrgFluxModified
    real(rkind),intent(inout) :: sumBalance(:), balance(:)
    real(rkind),intent(in) :: dtSubstep
    real(rkind),intent(inout) :: sumCanopyEvaporation, sumLatHeatCanopyEvap, sumSenHeatCanopy
    real(rkind),intent(in) :: scalarCanopyEvaporation_temp,scalarLatHeatCanopyEvap_temp,scalarSenHeatCanopy_temp
    real(rkind),intent(in) :: scalarCanopyEvaporation_data,scalarLatHeatCanopyEvap_data,scalarSenHeatCanopy_data
    real(rkind),intent(inout) :: sumSoilCompress, scalarSoilCompress
    real(rkind),intent(inout) :: sumLayerCompress(:), mLayerCompress(:)
    logical(lgt),intent(in) :: fluxMask(:)
    integer(i4b),intent(inout) :: fluxCount(:)
    real(rkind),intent(inout) :: flux_mean(:)
    real(rkind),intent(in) :: flux_temp(:)
    real(rkind),intent(in) :: dt

    integer(i4b) :: ixLayer, iSoil, iVar
    real(rkind) :: dt_wght
      ...
      if(nrgFluxModified .or. ixVegNrg/=integerMissing)then
        sumCanopyEvaporation  = sumCanopyEvaporation  + dtSubstep*scalarCanopyEvaporation_temp  ! canopy evaporation/condensation (kg m-2 s-1)
        sumLatHeatCanopyEvap  = sumLatHeatCanopyEvap  + dtSubstep*scalarLatHeatCanopyEvap_temp  ! latent heat flux for evaporation from the canopy to the canopy air space (W m-2)
        sumSenHeatCanopy      = sumSenHeatCanopy      + dtSubstep*scalarSenHeatCanopy_temp      ! sensible heat flux from the canopy to the canopy air space (W m-2)
      else
        sumCanopyEvaporation  = sumCanopyEvaporation  + dtSubstep*scalarCanopyEvaporation_data  ! canopy evaporation/condensation (kg m-2 s-1)
        sumLatHeatCanopyEvap  = sumLatHeatCanopyEvap  + dtSubstep*scalarLatHeatCanopyEvap_data  ! latent heat flux for evaporation from the canopy to the canopy air space (W m-2)
        sumSenHeatCanopy      = sumSenHeatCanopy      + dtSubstep*scalarSenHeatCanopy_data      ! sensible heat flux from the canopy to the canopy air space (W m-2)
      endif  ! if energy fluxes were modified

      ! get the total soil compression
      if (nSoil>0) then
        ! scalar compression
        sumSoilCompress = sumSoilCompress + dtSubstep*scalarSoilCompress ! total soil compression
        ! vector compression
        do iSoil=1,nSoil
          if(ixSoilOnlyHyd(iSoil)/=integerMissing)&
          sumLayerCompress(iSoil) = sumLayerCompress(iSoil) + dtSubstep*mLayerCompress(iSoil) ! soil compression in layers
        end do
      endif

     ! increment fluxes
      dt_wght = dtSubstep/dt ! define weight applied to each sub-step
      do iVar=1,size(flux_mean)
        if(fluxMask(iVar)) then

          ! ** no domain splitting
            flux_mean(iVar) = flux_mean(iVar) + flux_temp(iVar)*dt_wght
            fluxCount(iVar) = fluxCount(iVar) + 1

module hyp_2F1_module
   ...
   contains
    ...
    !----------------------------------------------------------------------
    !
    attributes(device,host) function INF_NORM(Z)
    ...
    !
    attributes(device,host) function TANZ(Z)
      complex(rkind),intent(in) :: Z
      complex(rkind) :: TANZ
      TANZ=SIN(Z)/COS(Z)
      return
    ...
    !
    attributes(device,host) function LOG1P(Z)
      complex(rkind),intent(in) :: Z
    ...
    !
     attributes(host,device) function EXPM1(Z)
      complex(rkind),intent(in) :: Z
    ...
    !
    !----------------------------------------------------------------------
    attributes(host,device) function LOG_GAMMA_FUN(Z_in) result(RES)
      !----------------------------------------------------------------------
    ...
    ! res: returned value
    !----------------------------------------------------------------------
    attributes(host,device) function GAMMA_INV(Z_in) result(RES)
      !--------------------------------------------------------------------
      implicit none
      complex(rkind),intent(in) :: Z_in
      integer(i4b)              :: N,I
      ...
      if(X.ge.HALF) then
         ...
         RES=EXP(Z_P_G_M0P5-Z_M_0P5*LOG(Z_P_G_M0P5)-LOG_SQRT_2PI) &
              /GAMMA_SUM
         if (looped) RES = upper/RES
         return
      else
         X=REAL(Z,rkind); N=NINT(X)
         EPS=Z-N
         if(MOD(N,2).eq.0) then
            upper = SIN(M_PI*EPS)*M_1_PI
    ...
    ! res: returned value
    !----------------------------------------------------------------------
    attributes(host,device) function GAMMA_RATIO_DIFF_SMALL_EPS(Z_in,EPS_in) result(RES)
      !--------------------------------------------------------------------
      implicit none
      complex(rkind),intent(in) :: Z_in,EPS_in
      integer(i4b)              :: N,M,I
      ...
      if((X.ge.HALF).or.(EPS_PX.ge.HALF)) then
         ...
         if(EPS.ne.ZERO) then
            RES=EXPM1(Z_M_0P5*LOG1P(EPS/Z_PG_M0P5) &
                 ...
                 /EPS
                 IF (EPS.ne.ZERO .and. looped) then
                  RES = A*RES + B
                  RES = RES/(ONE - (-EPS*RES))
               else if (looped) then
                  RES = RES + B
    ...
    !
    !----------------------------------------------------------------------
    attributes(host,device) function GAMMA_INV_DIFF_EPS(Z,EPS)
      !--------------------------------------------------------------------
      implicit none
    ...
    ! one_meps: 1-eps
    !----------------------------------------------------------------------
    attributes(host,device) function A_SUM_INIT(M,EPS,GAMMA_INV_ONE_MEPS)
      !--------------------------------------------------------------------
      implicit none
    ...
    !----------------------------------------------------------------------
    attributes(host,device) function LOG_A_SUM_INIT(M,EPS)
      !--------------------------------------------------------------------
      implicit none
      integer(i4b),intent(in)   :: M
    ...
    !----------------------------------------------------------------------
    attributes(host,device) function B_SUM_INIT_PS_ONE(A,B,GAMMA_C,GAMMA_INV_ONE_MEPS, &
    ...
    !----------------------------------------------------------------------
    attributes(host,device) function B_SUM_INIT_PS_INFINITY(A,C,GAMMA_C,GAMMA_INV_CMA, &
    ...
    !----------------------------------------------------------------------
    attributes(host,device) subroutine CV_POLY_DER_TAB_CALC(A,B,C,Z,CV_POLY_DER_TAB)
      !--------------------------------------------------------------------
      implicit none
    ...
    !----------------------------------------------------------------------
    attributes(host,device) function CV_POLY_DER_CALC(CV_POLY_DER_TAB,X)
      !--------------------------------------------------------------------
      implicit none
    ...
    !----------------------------------------------------------------------
    attributes(host,device) function MIN_N_CALC(CV_POLY_DER_TAB)
      !--------------------------------------------------------------------
      implicit none
      real(rkind),intent(in) :: CV_POLY_DER_TAB(0:3)
    ...
    !----------------------------------------------------------------------
    attributes(host,device) function HYP_PS_ZERO(A,B,C,Z)
      !--------------------------------------------------------------------
      implicit none
      complex(rkind),intent(in) :: A,B,C,Z
    ...
    !----------------------------------------------------------------------
    attributes(host,device) function HYP_PS_ONE(A,B,C,MZP1)
      !--------------------------------------------------------------------
      implicit none
    ...
    !----------------------------------------------------------------------
    attributes(host,device) function HYP_PS_INFINITY(A,B,C,Z)
      !--------------------------------------------------------------------
      implicit none
      complex(rkind),intent(in) :: A,B,C,Z
      integer(i4b)              :: N,M,PHASE,M_M2,MIN_N,M_P1
    ...
    !----------------------------------------------------------------------
    attributes(host,device) function HYP_PS_COMPLEX_PLANE_REST(A,B,C,Z)
      !--------------------------------------------------------------------
      implicit none
      complex(rkind),intent(in) :: A,B,C,Z
      integer(i4b)              :: N
    ...
    ! res: returned result
    !----------------------------------------------------------------------
    attributes(host,device) function HYP_2F1(A,B,C,Z) result(RES)
      !--------------------------------------------------------------------
      implicit none
      complex(rkind),intent(in) :: A,B,C,Z
    ...
    attributes(host,device) function HYP_2F1_new(A_in, B_in, C_in, Z_in) result (RES)
      implicit none
      complex(rkind),intent(in) :: A_in, B_in, C_in, Z_in
      complex(rkind) :: scale
      complex(rkind) :: A, B, C, Z, a_tmp, b_tmp, c_tmp, z_tmp
      integer(i4b)              :: NA,NB,NC,I
      real(rkind)               :: RE_A,RE_B,RE_C,ABS_Z,ABS_ZM1,ABS_Z_OVER_ZM1
      real(rkind)               :: ABS_ZM1_OVER_Z,ABS_ZM1_INV,R_TABLE(1:5),R,ABS_Z_INV
      complex(rkind)            :: RES,Z_SHIFT,Z_OVER_ZM1,ZM1
      ...
      do
         ...
         if(IS_C_NEG_INT) then
            ...
            if(IS_A_NEG_INT.and.(NC.lt.NA)) then
               if((Z.eq.ONE).or.(ABS_Z.lt.ABS_Z_OVER_ZM1)) then
                  RES=scale * HYP_PS_ZERO(A,B,C,Z)
                  return
               else
                  RES=scale * ((-ZM1)**(-A))*HYP_PS_ZERO(A,C-B,C,Z_OVER_ZM1)
                  return
               endif
            else if(IS_B_NEG_INT.and.(NC.lt.NB)) then
               if((Z.eq.ONE).or.(ABS_Z.lt.ABS_Z_OVER_ZM1)) then
                  ...
                  RES=scale * ((-ZM1)**(-B))*HYP_PS_ZERO(B,C-A,C,Z_OVER_ZM1)
                  return
               endif
            else
               print*,'2F1 UNDEFINED'
         ...
         ABS_ZM1=ABS(ZM1)
         if(ABS_ZM1.lt.1.e-5_rkind) then 
            RES=scale*HYP_PS_ONE(A,B,C,-ZM1)
            return
         endif
         ABS_Z=ABS(Z); ABS_Z_OVER_ZM1=ABS_Z/ABS_ZM1; ABS_Z_INV=ONE/ABS_Z
         ABS_ZM1_OVER_Z=ONE/ABS_Z_OVER_ZM1; ABS_ZM1_INV=ONE/ABS_ZM1
         ...
         do I=1,5
            ...
            if(IS_CMB_SMALL.and.(ABS_Z_OVER_ZM1.le.R)) then
               RES=scale*((-ZM1)**(-A))*HYP_PS_ZERO(A,C-B,C,Z/ZM1)
               return
            endif
         enddo
         do I=1,5
            R=R_TABLE(I)
            if(ABS_Z_INV.le.R) then 
               RES=scale*HYP_PS_INFINITY(A,B,C,Z)
               return 
            endif
            if(IS_CMB_SMALL.and.(ABS_ZM1_OVER_Z.le.R)) then 
               RES=scale*((-ZM1)**(-A))*HYP_PS_INFINITY(A,C-B,C,Z/ZM1)
               return
            endif
            if(ARE_ABC_SMALL.and.(ABS_ZM1.le.R)) then 
               RES=scale*HYP_PS_ONE(A,B,C,-ZM1)
            ...
            if(ARE_A_CMB_C_SMALL.and.(ABS_ZM1_INV.le.R)) then 
               RES=scale*((-ZM1)**(-A))*HYP_PS_ONE(A,C-B,C,-ONE/ZM1)
               return
            endif
         enddo
         RES=scale*HYP_PS_COMPLEX_PLANE_REST(A,B,C,Z)
         return
      enddo
   end function HYP_2F1_new

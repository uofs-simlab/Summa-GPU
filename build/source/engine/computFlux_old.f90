...
subroutine computFlux(&
  ...
  real(rkind),device :: mLayerMatricHeadTrial_d(size(mLayerMatricHeadTrial),nGRU)
  real(rkind),device :: mLayerTempTrial_d(size(mLayerTempTrial),nGRU)
  real(rkind),device :: mLayerVolFracLiqTrial_d(size(mLayerVolFracLiqTrial),nGRU)
  real(rkind),device :: mLayerMatricHeadLiqTrial_d(size(mLayerMatricHeadLiqTrial),nGRU)
  real(rkind),device :: mLayerVolFracIceTrial_d(size(mLayerVolFracIceTrial),nGRU)
  integer(i4b) :: nGRU = 5
  ...
  real(rkind),device :: dHydCond_dMatric_d(size(dHydCond_dMatric),nGRU)
  ! -------------------------------------------------------------------------------------------------------------------------
  ...
  associate(nSoilOnlyHyd => indx_data%var(iLookINDEX%nSoilOnlyHyd)%dat(1)) ! intent(in): [i4b] number of hydrology variables in the soil domain
    if (nSoilOnlyHyd>0) then ! if necessary, calculate the liquid flux through soil
      ...
      mLayerMatricHeadTrial_d(:,1) = mLayerMatricHeadTrial
      mLayerTempTrial_d(:,1) = mLayerTempTrial
      mLayerVolFracLiqTrial_d(:,1) = mLayerVolFracLiqTrial
      mLayerVolFracIceTrial_d(:,1) = mLayerVolFracIceTrial
      mLayerMatricHeadLiqTrial_d(:,1) = mLayerMatricHeadLiqTrial
      dHydCond_dMatric_d(:,1) = dHydCond_dMatric
      call allocate_device_decisions(decisions)
      call allocate_device_param_data(mpar_data_d,mpar_data,nGRU)
      ...
      call allocate_device_deriv_data(deriv_data_d,deriv_data,nGRU,nSoil,nLayers,nSnow)
      call soilLiqFlx(firstSplitOper,nSoil,decisions,nGRU,mLayerTempTrial_d,mLayerVolFracLiqTrial_d, mLayerVolFracIceTrial_d, mLayerMatricHeadLiqTrial_d, mLayerMatricHeadTrial_d,&
      mpar_data_d,indx_data_d,prog_data_d,diag_data_d,flux_data_d,deriv_data_d,dHydCond_dMatric_d,err,cmessage)
      ...
      call finalize_device_diag_data(diag_data_d,diag_data,nSnow)
      ...
      call finalize_device_deriv_data(deriv_data_d,deriv_data)
      call deallocate_device_deriv_data(deriv_data_d)
      mLayerMatricHeadTrial = mLayerMatricHeadTrial_d(:,1)
      mLayerTempTrial = mLayerTempTrial_d(:,1)
      mLayerVolFracLiqTrial = mLayerVolFracLiqTrial_d(:,1)
      mLayerVolFracIceTrial = mLayerVolFracIceTrial_d(:,1)
      mLayerMatricHeadLiqTrial = mLayerMatricHeadLiqTrial_d(:,1)
      dHydCond_dMatric = dHydCond_dMatric_d(:,1)
